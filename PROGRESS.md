...
- Phase 2 = **Increment G (G1→G11)**; **G1→G5 all MERGED** (G5b2b closes G5 — see the G table below).
- **Last merged: G5b2b** — builder geo config editor + "Map" tab (PR #35 `787f3c7`). Authors can now set `capture_altitude` / `accuracy_threshold` / `default_center` / `default_zoom`; **G5 (geo) COMPLETE**.
- Latest gate counts: **Pest 676 + Vitest 352**, all 6 CI jobs green.
- `main` clean, branch deleted, local `main` synced. Nothing in-flight.

## Next Session — Resume Here

**PRIMARY NEXT PROMPT →** `Read PROGRESS.md and build Increment G6` — media capture (image / audio / video):
- build out the `attachments` write path (upload + storage + RLS-scoped retrieval)
- new field-type renderers wired through the same `FieldInput.vue` / public-runtime mirror as the other Increment-G types
- respect the frozen `EngineValue` contract (attachments ride the answer/config objects; no grammar bump expected)

Plan: `C:\Users\DOH\.claude\plans\read-progress-md-and-start-shimmying-yeti.md` (full G-map; §G6).
...
| **Phase 2 — Kobo-style rigor + offline** | Repeat groups, matrix/grid, Likert, cascading select, geo (PostGIS), media capture, full expression engine, XLSForm import/export, PWA/offline, field library/templates | 🚧 **In progress** — Increment G (G1→G11); through G5 (geo) COMPLETE, next = G6 |
...
- **G6** — Media capture: image / audio / video (build out the `attachments` write path)
