...
- Tenant Owner/Admin view shows: total forms, total submissions (current period vs. prior period), count of forms currently accepting responses, a submission-volume trend, and the top forms by submission volume.
- Form Owner/Editor view shows: submissions over time for that form, a completion rate where measurable (started vs. completed), average completion time, and a breakdown by submission channel (manual / guest / OCR / API / offline-sync).
...
- Cross-form aggregation and filtering within a tenant (e.g., all forms tagged to one program or campaign).
...
- Advanced cross-form analytics is a paid-tier feature gate, per the Pricing & Feature-Gating Matrix (doc #24).
...
- Email delivery uses the transactional email provider already modeled in the architecture (invites/alerts); real-time in-app delivery reuses the tenant-namespaced Reverb channel.
...
- **Build-vs-buy for the form-rendering engine is still open.** The plan calls for a timeboxed Phase 0 spike comparing SurveyJS/Form.io against a custom-built engine, resolved via an Architecture Decision Record (ADR) — this PRD assumes a decision will be made before Phase 1 field-type work begins in earnest, but the decision itself is not yet made.
...
- **Hosting cost is a planning estimate, not a quote.** The recommended launch platform (Laravel Cloud) carries an indicative €80–150/month reference point for a mid-size production setup; this should be re-verified against current pricing before it is used in any budget commitment.
