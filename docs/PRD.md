...
The legacy system and the two market references (KoboToolbox, Fillout.com) are sources of **proven ideas**, not blueprints to copy. Concretely:
...
- **Gamified** — light progress affordances (a getting-started checklist with a visible progress meter) in Phase 1; a full achievements system (points, badges, streaks, team progress) is wanted but deliberately scheduled as the final increment before production deployment, so it is designed once against the finished feature set rather than retrofitted per feature. **BUILDING since 2026-08-17** — the engine is specified in the Gamification Design Doc (#28) and its substrate in ADR-0020, with the award ledger, the seven-rule scoring vocabulary and the eight listeners shipped in K1a, the badge catalog and its ledger in K1b, and the derived streak, workspace team progress and the one-shot history backfill in K1c — which also closed a live defect it found on the way: an invited member who accepted had been earning no join points and no welcome badge at all. Three product decisions of record were taken the same day and are **not to be re-asked**: it is **free on every plan tier** (a standing principle, not a commercial gate); the **named leaderboard reuses `dashboard.org.view`** rather than minting a thirtieth permission, with every member seeing their own standing; and existing workspaces are **backfilled from the `audits` ledger** rather than starting at zero.
...
|---|---|---|---|---|
| **G1 — Fast time-to-value** | A brand-new tenant can go from signup to a published, shareable form with no help/support ticket *(as built, a workspace is created by an operator with `tenants:create`, not at signup — see the self-serve workspace question in `docs/claims/decisions.md`)* | Time-to-first-published-form (TTFPF) | Median < 15 minutes | Phase 1 |
...
- Form Owner/Editor view shows: submissions over time for that form, a **draft-conversion rate** and the **median time from first save to submission** — both on forms with save-and-resume enabled only, both stating their denominator on the face of the tile, and both suppressed rather than estimated for any period extending beyond the tenant's draft-retention window — and a breakdown by submission channel (manual / guest / OCR / API / offline-sync). *(Shipped: I10c — the channel breakdown on the ungated `/dashboard`, and `/forms/{form}/analytics` for the per-form view, both on every tier. The two draft metrics are ADR-0011 §D5's, amended there. The per-form page carries `can:view,form` and deliberately NO `feature:advanced_analytics`: PRD:198 is a Phase-1 criterion, and §D9's amendment records the five structural mechanisms that stop it drifting into a second `/analytics`.)*
...
Acceptance criteria:
- **Registration/access control**: a tenant-level toggle for whether new members can self-register or must be invited, plus a separate platform-level toggle (super-admin only) for whether new tenant signup is currently open. ⚠️ *As built, that platform switch (`SettingKey::RegistrationOpenSignup`) gates new **accounts**, not new workspaces:* a central-host registration joins no workspace, and a workspace is created by the operator command `tenants:create` (`M95`).
...
> ~~**Decision (not pinned by the plan):** the exact client-side screenshot-capture technique (the browser's native Screen Capture API with a user permission prompt, versus a DOM-snapshot library such as html2canvas) is left open pending a small technical spike~~ — **CLOSED 2026-08-07 by `docs/adr/0015-feedback-screenshot-capture.md` (Increment I7a): the native Screen Capture API, with a file input rendered beside it at all times.** The spike's finding is that the trade is not evenly balanced in this product: a DOM snapshot renders the Leaflet map picker (ADR-0006) and the signature-capture `<canvas>` blank — the two surfaces a bug report is most likely to be about — and would add a ~1 MB production dependency, last released in 2022, to the set CI's `npm audit --omit=dev` gate blocks merges on. The permission prompt is the accepted price, and the always-present file input is what keeps a decline, or a browser without the API (iOS Safari), from costing the reporter anything. See the ADR for the eight sub-decisions, including why SVG is excluded more sharply here than for a brand logo.
>
> **AS BUILT (I7a).** The feature is closed end-to-end: capture → the shared `attachments` table (`AttachmentKind::FeedbackScreenshot`) → the tenant's own read-only list at `/feedback` (gated on the `feedback.view` permission the role catalog had carried since Phase 0 with no code behind it) → the platform support console at `/admin/feedback`, whose New → Reviewed → Resolved transitions are audited into the **reporting workspace's own** ledger (RBAC §9 transparency), never the operator's.
