...
`pdo_pgsql` extension and no `rolldown` win32 binding — so every command below is `docker compose exec`. ⚠️ **This chapter is the local Docker stack only. A testing server has no demo accounts**: it runs `APP_ENV=production`, so the demo seed never runs there, and a tester signs in with the account their invitation created, at their workspace address `https://<slug>.<CENTRAL_DOMAIN>/login` — never the central host. The operator's side is `docs/deployment-infrastructure.md` §8.2. While a testing server redeploys, which it does after changes reach `main`, its pages briefly show a card saying the site is being updated: wait a moment and reload, and repeat once any action a page that was already open could not complete in that moment.
