...
| **Cache (Redis)** | ⚠️ **NOT BUILT AS DESCRIBED — corrected to as-built by Increment M46 (2026-08-29).** There is no Redis cache store in use and no tenant-prefixed cache namespacing: `config/cache.php` defaults to `CACHE_STORE=database`, no dashboard-KPI caching exists to be prefixed, and the **only** `tenant:{…}` key anywhere in `app/` is a queue rate limiter in `QueueServiceProvider` (`tenant:{id}:queue:{name}`, ADR-0007 §D9) — a fairness ceiling, not a cache prefix. | **The design intent stands** for the moment a cache is actually used for tenant-scoped reads: logical namespacing within one shared store, which is *not* physical isolation, and is an accepted trade-off consistent with the shared-DB posture — revisit only alongside a Phase 4 dedicated-infrastructure decision, never independently. ⚠️ **What the cache *is* used for today is three short-lived single-use markers, none of them tenant-scoped reads** — a connector refresh lock, an SSO replay guard and the guest proof-of-work replay guard — so the prefixing question has not yet been forced. **The first tenant-scoped cached read is what makes this row's mechanism owed, and it should not ship without it.** |
...
| **Realtime (Reverb)** | ⚠️ **NOT BUILT — corrected to as-built by Increment M46 (2026-08-29); this row previously described the control as if it existed.** There is no broadcasting stack in this repository: no `routes/channels.php`, no `config/broadcasting.php`, and no Reverb, Pusher or Echo dependency in `composer.json` or `package.json`. Reverb is **Track B by explicit user decision** — the same status `docs/data-dictionary.md` already records, and the reason is that it needs a long-lived service on a host that does not exist yet. | **The design intent stands and is what the control must be when the stack lands:** private/presence channel names namespaced by tenant (e.g. `private-tenant.{tenant_id}.dashboard`), with channel-authorization callbacks re-verifying the requesting user's tenant membership server-side, so a client cannot subscribe to another tenant's channel by guessing its name. ⛔ **Recorded as unbuilt rather than deleted, because the row was training readers the wrong way:** the Jobs row two rows up *was* rewritten to as-built, which taught anyone reading this table that an uncorrected row had been verified. Deleting this one would lose the requirement; leaving it as written asserted a control that cannot be relied on. |
...
**Amendment (Increment M8, 2026-08-20) — the PRE-AUTH role may hold a read carve-out too, and this ADR had never named that role at all.**
Every carve-out D3 discussed until now belonged to the **elevated** `meridian_superadmin` role. `meridian_auth` — the
NOBYPASSRLS login role the pre-authentication path uses — appeared nowhere in this document, even though it has held a
permissive `TO meridian_auth` SELECT on `users` since Phase 0 (`apply_users_rls.php`). M8 adds a second: `SELECT` on
`tenant_users`, plus the role-scoped policy `tenant_users_auth_select ... USING (true)`, generated by
`TenantIsolation::authRoleRead()` / `authRoleReadSql()` and pinned in `tests/Unit/TenantIsolationSqlTest.php` exactly as
every other shape is.

**Why it was worth spending.** `/invitations/{token}` runs with tenant context but no `auth`, and one arm of it writes a
password onto an existing `users` row. Deciding whether that arm is legitimate means asking whether the invitee's identity
has ever actually been used — and the only positive record of that, for a member who has changed their email address and
enrolled no second factor, is a `tenant_users` row **in another tenant**. Inside the invited tenant the question is
unanswerable rather than merely restricted: the strict shape plus `unique(tenant_id, user_id)` means the invite row is the
only row there, so the predicate would have returned false for everybody — the silently-zero-row failure this schema has
now recorded four times. Without the carve-out the fix would have shipped closing the reported hole and missing its largest
sub-case.

⛔ **THIS SPENDS A COST ADR-0019 §D1 EXPLICITLY DECLINED, AND THE TWO DECISIONS ARE NOT IN TENSION — READ BOTH.** §D1
refused to move Google's `sub` onto a second table, reasoning that *"a column rides the existing table-level GRANT … for
free; a second table needs a new GRANT and a new policy **on the most sensitive role in the deployment**"*. That reasoning
stands and is the reason this amendment exists rather than a migration landing quietly. The difference is what the cost buys:
§D1 was buying **generality for a second identity provider that scope had already ruled out** — a benefit worth nothing
today — whereas this buys the only available answer to a live `major` on an unauthenticated write path. A carve-out is not
forbidden; it is expensive, and it must be paid for by something real. **Anyone proposing a third should be held to the same
test, and §D1's refusal should not be read as a precedent that the answer is always no.**
...
checked before RLS, so a test of only the functional read would pass for the wrong reason), asserts that an ordinary tenant
connection still cannot see another tenant's memberships, and asserts that a write through the pre-auth role is refused.
...
- **Cheapest and fastest path at MVP/early scale.** One database to provision, migrate, back up, monitor, and tune. Onboarding a new tenant is ~~inserting one row into `tenants` (plus seed data)~~ inserting a handful of rows (corrected by `M95`: a working workspace is the `tenants` row with `owner_user_id`, its domain label, the Owner's `tenant_users` membership plus its role row, and a default subscription, written in one transaction — see the `tenants:create` command), not running a schema-creation or database-provisioning workflow — directly consistent with the plan's "start simple, add complexity only when justified" theme and its lean hosting posture (§1).
...
- Legacy schema audit — confirms the negative case directly: no `tenant_id` concept and no tenant-scoped authorization, plus the cautionary precedent of an **`id`-based super-admin convention repeated across several code layers**, which is exactly what this ADR's explicit `is_super_admin` boolean decision (D3) exists to avoid. ⚠️ **The exploitation detail that once sat here was removed by `M51` (`D6`).** The *pattern* is the transferable lesson and it is kept in full; an audit of how a specific third-party system could be compromised is not this repository's to publish.
