...
| `plans` given `tenant_id`+RLS by reflex, hiding the global catalog per-tenant | §D1/§Method note and `data-dictionary.md:604-605` state `plans` is RLS-exempt with no `tenant_id`; migration-lint short-circuits on a table with no literal `tenant_id`, and adding an `EXEMPT_TABLES` entry is explicitly wrong |
