...
**§D10 — the shared route needs the OWNER too, not only the kind (M33, 2026-08-26).** §D9 closed the kind and said in terms that it closed *one kind, not the class*. This closes the class. `AttachmentPolicy::view()`'s default arm required only `submissions.view` where {@see SubmissionPolicy::view()} requires that permission **and** org-wide visibility or per-form collaboration or being the respondent — so `GET /attachments/{attachment}` read any stored object in the tenant by id with no per-form check at all, while every surface that *lists* those objects is scoped. The affected roles were exactly `form_editor` and `reviewer`: `hasOrgWideVisibility()` is `dashboard.org.view`, which owner, admin and viewer hold, so for those three `SubmissionPolicy` already granted tenant-wide and there was no gap to close.

⛔ **The sharpest consequence was that revocation did not revoke.** Remove a `form_editor` from a form's collaborators and `SubmissionPolicy` refuses them the submission on the next request — while every attachment id they had ever seen kept working through this route, indefinitely. "Hard to guess" is not a mitigation when the ids were legitimately theirs once. The reachable set was not small: respondent uploads and media captures, the per-submission PDF export (`is_pii = true`, and it renders the whole answer document), and archived webhook delivery envelopes — the last written `ScanStatus::Skipped`, which `servable()` admits, **under a comment asserting they are "never served to a browser" that this route made false.**

**The decision: the kind chooses the permission, and where the kind is submission-domain data the OWNER chooses the scope.** The `match` is now exhaustive over all nine kinds with **no `default` arm**, so PHPStan reports a tenth kind as unhandled rather than absorbing it — a default arm is precisely what absorbed the seventh and produced §D9. Three kinds are answered on a permission alone because they have no form to be scoped to: the feedback screenshot on `feedback.view` (§D9, unchanged); a **brand logo** on any member, deliberately *not* narrowed because `GET /branding/logo` serves the same bytes unauthenticated to email clients, so tightening this route would protect nothing already private; and an **archived webhook envelope** on `webhooks.manage`. The five submission-domain kinds take `submissions.view` **and** their owner's scope. `Avatar` — wired by nothing today — fails closed, because a submission scope is meaningless for a file owned by a user and a future avatar feature must decide here rather than inherit.

⚠️ **The webhook envelope is a narrowing rather than a scoping, and it is a product decision rather than a mechanical one.** An envelope is the full payload of whatever form fired it, so it crosses every per-form boundary at once; there is no form to scope it to, and leaving it on `submissions.view` would have kept it readable by all five roles. `webhooks.manage` (Owner/Admin) is the authority that configures the endpoint the payload was sent to. **Filed as `D4` in `docs/claims/decisions.md` with its revert path named**, on the §D3/M26 precedent — proceeding on the recommendation rather than blocking, because the alternative is leaving a live cross-form read open while a question waits.

⚠️ **THE OWNER IS RESOLVED LOCALLY, AND THE REPOSITORY REQUIRED THAT BEFORE THIS INCREMENT ASKED.** The obvious implementation — walk `$attachment->attachable` through the morph map — **cannot work and must not be made to work.** `attachable_type` carries five aliases and only three are registered; `tenant` (§D6's sibling, the brand logo) and `feedback_report` (§D6's own) are deliberately absent, because registering them would change how Sanctum's `tokenable_type` and Spatie's `model_type` serialize and split existing rows between alias and FQCN — the `enforceMorphMap` break that cost 90 test failures. `BrandingMorphAliasTest` pins that absence *and prescribes this remedy by name*: **"a LOCAL resolution (a match on `kind`, or a dedicated relation), never a global registration."** Nothing in the policy touches `$attachment->attachable`; an alias it does not recognise fails closed.

**The submission arm DELEGATES rather than copying the predicate**, through `$user->can('view', $submission)`. Re-implementing "org-wide or collaborates or respondent" here would create a second copy free to drift from the first — the exact divergence `Submission::scopeVisibleTo()`'s mirror-pin already exists to prevent between the single-row check and the list query. This makes a third surface agree by construction instead of by review, and the test that proves it is the respondent case: it passes only because the third arm came along with the delegation, and a hand-copied predicate that forgot it would satisfy every other case in the file.

**What this changes for a legitimate user: nothing.** The "your PDF is ready" notification (`GeneratePdfJob`) links to this route, and its recipient is whoever requested the export — who necessarily passed `SubmissionPolicy::export()`, the same scope plus `submissions.export`. The links that stop working are exactly the ones held by someone since removed from the form, which is the defect.
