...
- ~~**A real extraction requirement — a tenant offboarding, a GDPR Article 20 request, or a customer with an isolation clause.**~~ ✅ **DISCHARGED 2026-08-14 by P2b / ADR-0018.** Both entry criteria are answered, and both turned out to follow from decisions already on the record rather than needing a product call. Kept below because the *questions* are what the next reader needs, and because one of the two answers is the opposite of what this section implied.
  1. **What does a tenant's extract contain for `users`?** → **A membership roster.** ADR-0018 §D3 withholds nine columns, each with a reason: four credentials for a **central** identity, `last_active_tenant_id` (another workspace's UUID), `is_super_admin`, and the two platform-agreement timestamps. The dangling foreign key this bullet predicted is **real and is now reported rather than removed** (§D4): an invitation, a removed member and the platform operator named by `impersonation_tokens.operator_id` all land there legitimately, so the artefact states them instead of pretending completeness. ⚠️ The fix this bullet's phrasing invites — widening the GUC so the roster is "complete" — was **refused**, on the same grounds J3c1 refused it: `app.current_user_id` names an authenticated person, and it would resolve the operator, who belongs to no tenant.
  2. **Are platform-shared rows in or out?** → **Out, and counted.** The widened six are read under an explicit `tenant_id = ?` on top of RLS, which is the one place this codebase knowingly writes the predicate ADR-0002 spent a decision removing. `rlsReturnsSuperset()` is what drives that branch, so it is load-bearing rather than documentary.
