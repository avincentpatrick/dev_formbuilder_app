...
> ✅ **THE 0017 COLLISION THIS NOTE WARNED ABOUT IS RESOLVED, AND EVERY "ADR-0017" IN THIS DOCUMENT IS NOW UNAMBIGUOUS.** For a short window two ADRs carried the number: this one's predecessor, `0017-tenant-isolation-tiering.md`, and Lane A's `0017-first-party-google-sign-in.md` (J3c2 #151). Neither lane erred — each checked the sequence before starting and each correctly saw 0016 as the highest. By arrival order Lane A's moved, and it is now **`0019-first-party-google-sign-in.md`**; nothing in *this* document changed, because every reference here always meant the tiering ADR. ⚠️ **0010 was not the answer even though the sequence skips it** — `0011`, `0012` and `0013` each reserve it for H1d, the OCR provider bake-off. The generalisable rule the collision taught, which outlives it: **a globally-numbered artefact is a shared-namespace allocation and cannot be chosen safely from inside one lane** — reading the current maximum is not a reservation, and the same session also produced a duplicate migration prefix for exactly that reason.
...
> **This is the answer to ADR-0017's first entry criterion: `users` is a membership roster.** Ten columns withheld. Four are credentials for a **central** identity (Context §2 — one human, N workspaces), so extracting them hands one tenant material for an account still live in workspaces it has nothing to do with. `last_active_tenant_id` is *another workspace's uuid*, which is the single cross-tenant fact this whole architecture exists to withhold. `is_super_admin` describes the platform's staff. `tos_accepted_at` and `privacy_policy_accepted_at` record an agreement between the person and the platform operator, to which a tenant is not a party.
>
> ⚠️ **The tenth is `google_id`, it was added AFTER this ADR was accepted, and how it got here is the part worth keeping.** It arrived with first-party Google sign-in (Increment J3c2) in the other lane, while `TenantExtractColumnDriftTest` arrived here in P2b. Each merged green on a base that did not contain the other, so the integration branch went red on the census the moment they met — and until it was fixed the column had **no withholding decision at all**. It gets one now on `last_active_tenant_id`'s argument in a stronger form: that column discloses *that* a member belongs elsewhere, whereas `google_id` is a stable, globally unique **join key**, so two tenants each holding an extract could match rows on it and establish exactly *which* members they share, with neither operator ever being told.
>
> The same increment added `google_auth_requests`, which is extracted rather than skipped — a tenant may legitimately see *that* a sign-in was attempted against its workspace and when, exactly as it may for `impersonation_tokens` — with three columns withheld: `handoff_hash` (a live single-use credential; the preimage completes that sign-in), `state_id` (the correlator naming an in-flight, still-redeemable request), and `google_sub` (the same globally unique join key as `google_id`, on a second table).
>
> **The generalisable point, and it is about process rather than columns: a decision list guarded only by a shape test is guarded against the wrong thing.** The census would have gone green the instant somebody pasted the new column in — which is why the file's own instruction is *"decide first, then paste"*, and why the decision is now pinned by name in `it('withholds every credential column on users')` rather than by count.
