...
- Formal build-vs-buy justification for the form-rendering engine → the **Phase 0 spike ADR** (`docs/adr/0004-form-rendering-engine-build-vs-buy.md`, executed via `docs/spikes/form-engine-spike-plan.md`; this document assumes the "build custom" path as its working baseline, per the plan's default trajectory, and is explicitly contingent on that ADR's outcome — see Risk R7 in §9).
...
| Queue Workers (Horizon) | Async execution of pipeline post-persist stages, exports, OCR extraction calls, webhook delivery, notification sending | Horizon gives per-queue observability (depth, throughput, failed jobs) required by the Observability doc. |
| Scheduler | Cron-driven periodic jobs: webhook retry sweep, usage-counter rollups, subscription reconciliation, data-retention purges | Runs `schedule:run` every minute; each scheduled task itself dispatches to the queue rather than running inline, so a slow task can't block the scheduler tick. |
...
| Redis | Queue broker (Horizon), cache, rate limiting, webhook idempotency dedupe cache, Reverb pub/sub backplane | Single Redis instance is sufficient at MVP scale; can be split (cache vs. queue vs. pub/sub) later without app changes since each use goes through its own Laravel config connection name. |
...
| 5 | **Background jobs / queue** | `tenant_id` is serialized into every queued job's payload; a job middleware re-establishes the RLS session variable before `handle()` runs. | Queue workers execute outside the HTTP request lifecycle, so without this a job silently runs with no tenant context — or worse, leftover context from a previous job on a reused worker process. |
...
| 7 | **Cache & rate limiting (Redis)** | Cache keys namespaced `tenant:{tenant_id}:...`; rate-limiter buckets are per-tenant, never global. | Cache-key collisions leaking data between tenants; one noisy tenant exhausting a shared rate-limit budget that starves every other tenant. |
...
| Public form schema | `GET /api/v1/public/f/{shareToken}` | Fetch the pinned `form_version` schema for rendering | Public Runtime (guest) |
| Submissions | `POST /api/v1/public/f/{shareToken}/submissions` (guest), `POST /api/v1/forms/{form}/submissions` (manual/API), `GET/PATCH/DELETE /api/v1/submissions/{submission}` | Create/list/review submissions through the unified pipeline (§4.1) | All caller types |
...
- **Retry with exponential backoff and jitter**: driven by a `next_retry_at` column swept by a scheduled job (never a naive immediate re-queue loop).
...
| Form-rendering engine | Custom (working baseline), pending a timeboxed **Phase 0 spike** evaluating SurveyJS/Form.io | Neither off-the-shelf library natively speaks this project's specific blend of XLSForm expressions + repeat instances + Kobo/Fillout hybrid UX; final call is via ADR-0004 after the spike (`docs/spikes/form-engine-spike-plan.md`), not by default (see Risk R7). |
...
| Queue / cache | **Redis + Laravel Horizon** | Legacy ran with "no Redis in active use" — a real gap given webhooks, exports, and sync all need reliable, observable async processing. |
...
| R6 | Webhook/export/OCR async infrastructure becomes a cost or noisy-neighbor surprise at scale (usage-sensitive managed hosting) | Unexpected hosting cost growth, or one high-volume tenant degrading queue latency for every other tenant | Per-tenant rate limits and circuit breakers (§7.4); Horizon queue-depth metrics actively monitored; plan tiers gate webhook volume/export frequency; hosting choice revisited via its own ADR if costs exceed projections. |
| R7 | The build-vs-buy decision for the form-rendering engine (custom vs. SurveyJS/Form.io) is explicitly deferred to a Phase 0 spike | Parts of §4.3 (custom Expression Evaluator) and the field-type catalog architecture assume "build"; a spike outcome favoring "buy" would invalidate specific sections of this document | This document explicitly flags §1.3, §4.3, and §8's form-rendering-engine row as contingent on **ADR-0004**'s outcome (`docs/adr/0004-form-rendering-engine-build-vs-buy.md`, run via `docs/spikes/form-engine-spike-plan.md`), and will be revised once the Phase 0 spike concludes — this is a known, bounded, and already-scheduled source of change, not an oversight. |
