...
| Scheduler | Cron-driven periodic jobs: webhook retry sweep, usage-counter rollups, subscription reconciliation, data-retention purges | ~~**Not yet implemented — specified by ADR-0007.** `routes/console.php` is stock and there is no `withSchedule`;~~ ✅ **Built in H2 (corrected by `M94`):** `routes/console.php` declares seven `Schedule::job(...)` entries — failed-job pruning, the usage-counter rollup, the expired-draft reaper, the scheduled-form sweep, the webhook retry sweep, connector token refresh and custom-domain verification — and deliberately no `withSchedule`; `schedule:run` runs every minute, each scheduled task dispatching to the queue rather than running inline, so a slow task can't block the scheduler tick. Subscription reconciliation and data-retention purges are not among the seven; the purge is the ledger row on NFR §8's soft-delete grace period. |
...
| PostgreSQL | System of record; RLS-enforced tenant isolation; JSONB for submission payloads **plus the typed `submission_answer_index` projection for queryable fields**; PostGIS for geo field types (Phase 2) | See §5 for RLS mechanics. **Corrected 2026-08-03 (ADR-0011):** this row previously read "JSONB+GIN". No GIN index exists on `submission_answers.answers` — the hybrid model's read path is carried by the typed projection and by whole-document reads. See ADR-0001's implementation-status note. |
...
>
> **Implementation status (2026-07-11).** The engine ships and is at **grammar v2.0** (`ExpressionEvaluator::GRAMMAR_VERSION` / `resources/public-runtime/engine` `GRAMMAR_VERSION`, held byte-identical by the shared `tests/golden/**` suite run through both engines — **180 expression + 114 validation vectors** — the validation suite moved off 106 in H7, the first increment since the corpora were frozen to deliberately EXTEND one (8 vectors pinning that a `hidden` field produces no respondent-facing error in either engine); per each suite's own `manifest.json`, which both runners assert per vector; H6a added a third, independently versioned corpus beside them, which H6b took dual-engine and grew to **76 template vectors**). Increment F2/F3 landed v1.0 (`= != > <`, `and`/`or`/`not()`, `selected()`); Increment G1 added repeat-instance scoping; **Increment G3 (PR #29) added the rest of the grammar below**: arithmetic `+ - * /`, the `>=`/`<=` comparators, and the value function library `if()`/`count()` (over G1 repeat instances + multi-select)/`int()`/`today()`/`now()`, plus `calculate` → `SemanticResult.computed` write-back. **One deviation from the design:** `today()`/`now()` read an injected clock on the evaluation context (an ISO-8601 string the caller stamps) so the two engines are deterministic and identical; and comparison (`> < >= <=`) is **numeric-only** — chronological ordering of ISO date strings is **deferred** (adding it would break the pinned "non-numeric operand → false" contract), so a date-range constraint like `${appt} >= today()` is not yet supported. **Phase 3's answer piping does not change this grammar, and H6a shipped without changing it.** It reuses the `${key}` lexeme inside *text* under a separate, versioned template grammar (Doc #26 §2, `TEMPLATE_VERSION` `1.0`) — a template yields text and participates in no evaluation, so `GRAMMAR_VERSION` stays `2.0` and both existing vector suites are untouched (verified at merge: `grammar_version` still appears at exactly 288 sites and neither corpus has a single changed byte). **The 288 figure moved to 296 in H7** — the expression and template corpora stayed byte-untouched, but H7 added 8 vectors to the *validation* corpus, each carrying `"grammar_version": "2.0"`. Read the invariant as written: the corpora are byte-untouched unless an increment deliberately extends one and says so. A changed count with no such note is drift. **Phase 3's workflow branching does not change this grammar either** (Doc #27 §1, ratifying the 2026-07-21 decision): branching is relevance-derived step-skipping over the existing section sequence, not a goto, so there is no `ExpressionKind::Branch`, no new AST node, no evaluator-mirror change and no new vectors — `ExpressionKind` stays at exactly three cases, `GRAMMAR_VERSION` stays `2.0`, and all four corpora stay byte-untouched at 296 `grammar_version` sites. ~~**One live deviation Doc #27 §3.4 records against this paragraph:** the injected clock is stamped by the PHP caller on every path, but the guest SPA's `toSemanticInput()` never passes one, so `today()`/`now()` resolve to ABSENT client-side while the server evaluates against a real clock.~~ **CLOSED in H21a**: `toSemanticInput()` gained an optional `now`, and `App.vue` stamps the device clock once per session via `isoClock()` — in PHP's exact `Carbon::toIso8601String()` shape (`2026-08-01T12:34:56+00:00`, seconds precision, explicit offset, never `Z`), **in UTC**, because the server re-validates at submit under its own UTC clock and a local offset would make `today()` disagree across the evening window in every UTC+ zone. The format is itself a mirror surface: both engines return the injected clock verbatim from `now()`, so `new Date().toISOString()` would have opened a fresh divergence. **Doc #27 amendment A1 corrects that section's motivating example in passing** — `${event_date} >= today()` was already always-false in BOTH engines, because ordering is numeric-only and `NUMERIC_RE` rejects an ISO date, exactly as this paragraph states two sentences above. What the fix actually restores is equality against the clock (`= today()` was true client-side precisely when the operand was EMPTY), `count()`, `if()` and `int()`.

Models XLSForm's `relevant`/`constraint`/`calculate` expression grammar (`${field}` references, `if()`, `selected()`, cross-repeat `count()`, `today()`, `now()`, arithmetic/comparison/logical operators), carried forward as a first-class engine per the plan — both because it is confirmed valuable in legacy and because it is a concrete Kobo/ODK/SurveyCTO migration lever via bidirectional XLSForm import/export. One grammar and one pair of engines (TypeScript/PHP, below) serves all three expression kinds, but each kind is stored in a different column per the Data Dictionary — **not** all three in `form_field_validations` as a single flat concept: `relevant` lives directly on `form_fields.relevant_expression` (and `form_sections.relevant_expression` for section-level skip logic — which is also, per Doc #27 §3, **the branching primitive**: a Phase-3 "branch" is a predicate on a section node, never an edge between nodes, so it adds no fourth storage location and no fourth expression kind), `constraint` lives in `form_field_validations.expression` (where it supersedes the structured `rule_type`/`operator`/`rule_value` columns when set), and `calculate` lives inside `form_fields.config` as the calculated-type field's formula reference.
...
| 13 | **Isolation tiering (escape hatch)** | Default posture is shared DB/shared schema. **Decided 2026-08-14 by ADR-0017**: should a driver appear, the target is an isolated **schema** in the same instance — a fully separate **database** is rejected for this schema, because login resolves an identity before any tenant exists (`/login` is routed `domain => null`, and `pgsql_auth` exists precisely because the join-shape RLS on `users` fails closed pre-context), and because three RLS policies subquery a neighbouring table, which PostgreSQL cannot express across databases. Not enabled: no bootstrapper runs and no connection is repointed. | Still not a leak-prevention layer. ⚠️ **The escape hatch is narrower than this row implied.** "`stancl/tenancy` supports promoting a tenant" is true of the package and not of this deployment: the stock `PostgreSQLSchemaManager` *replaces* `search_path` rather than prepending `public`, `tenants:migrate` is currently a green no-op pointed at a directory that does not exist, and the app role is `NOCREATEDB` by design. The path is a migration rather than a rewrite, as promised — but it is an increment, not a configuration flag. **P2b / ADR-0018 (2026-08-14) built the FIRST of the three deliverables this hatch depends on** — per-tenant export, as `php artisan tenants:extract`. The other two (import, and a per-tenant migration runner) remain unbuilt, so the hatch is one deliverable less theoretical and is still not reachable. |
...
2. **Publish is a single transaction** that:
   - Snapshots the draft's full structure into `schema_snapshot` (JSONB) on the version row — a denormalized, fast-to-retrieve copy used for rendering and offline manifests — **while the normalized `form_sections`/`form_fields`/`form_field_validations` rows remain independently queryable**, FK'd to that same `version_id`. Both representations are written together so they can never drift. **This snapshot is also, in full, the Phase-3 step model** (Doc #27 §6): the frozen per-section `relevant_expression`, `sequence`, `is_repeatable`, `min_instances`/`max_instances` and each field's `section_key` and sequences are everything a step projection consumes, so workflow branching adds **no new snapshot key and no new freeze step** — the H-map's "frozen into `schema_snapshot`" line item for H21a is discharged by construction.
...
4. **A published version's fields are never mutated in place.** Application-layer service methods refuse any update against a `form_fields`/`form_sections`/`form_field_validations` row whose `version_id` belongs to a non-draft version.
   > **Resolved in `docs/form-versioning-schema-migration.md` §2**: the database-level guard extends the existing RLS `INSERT`/`UPDATE`/`DELETE` policies on `form_sections`/`form_fields`/`form_field_validations` with an additional `EXISTS` predicate requiring the parent `form_versions.status = 'draft'`.
   > **Amended by H25 / ADR-0013 (2026-08-05) — the sentence that used to end this note ("not a separate trigger mechanism, so this schema keeps one single DB-level-guard idiom") was true of the CHILD tables and false as a general claim.** RLS covers the three content child tables, as built. It cannot cover the **`form_versions` row itself**, whose own policy leaves `UPDATE` deliberately status-blind so publish can flip `draft → published → superseded` — so every column of a published version was rewritable, including `status` back to `'draft'`, which re-opens all three child tables because their guard keys on exactly that value. A policy sees only OLD in `USING` and only NEW in `WITH CHECK`, and **no clause can compare them**, while a per-column immutability rule *is* an OLD-vs-NEW comparison; a CHECK cannot see OLD either. The parent row is therefore guarded by `form_versions_published_immutable_trg`, the schema's first trigger. The idiom rule is now stated rather than implied: **row-scoped invariants stay RLS; OLD-vs-NEW invariants get a trigger, and this is the only one that qualifies today** (ADR-0013 §D1).
...
| **Platform → Integration Consumer** (outbound webhook deliveries) | HMAC-SHA256 signature (`X-Webhook-Signature` header), computed from a per-endpoint shared secret + timestamp | The inverse trust direction from the rows above — lets the receiving system verify the delivery genuinely originated from the Platform. Secret rotation supported with a dual-secret grace period (no downtime). ⚠️ **During that window the header carries TWO comma-joined `sha256=<hex>` values, current first** — a receiver must split on `,` and accept if any matches, or it will reject every delivery from the moment a tenant first rotates. The wire format and the reasoning are in `docs/webhook-integration-design.md` §5. |
...
| Database | **PostgreSQL 16+** | Native JSONB + GIN indexing *(the capability the hybrid submission model was chosen for; no GIN index is **built** anywhere — see ADR-0001's implementation-status note, recorded 2026-08-03. J1b added one on each search vector and then **removed both, measured**: on an RLS table PostgreSQL will not promote a non-leakproof clause to an index qual, and `@@`, `LIKE` and `ILIKE` are all non-leakproof, so such an index is unreachable by construction rather than merely unused. This constrains every future indexing decision on an RLS table — see ADR-0001 and `SearchIndexUsageTest`)*, Row-Level Security (tenant-isolation backstop with no MySQL equivalent), PostGIS (legacy only stored geo as unindexed JSON strings). A deliberate departure from legacy's MySQL. |
...
| Hosting | **Self-hosted on the team's own Windows Server 2016** (a web server — **nginx** per ADR-0005, and **Apache 2.4 with `mod_proxy_fcgi`** on the testing site under `D46`, Doc #22 §8.3 — + PHP 8.4 FastCGI + self-managed PostgreSQL — a dedicated instance per site, PostgreSQL 15 on the testing site; no Redis or Memurai, since ADR-0007 put the queue, cache and sessions on the `database` driver; exactly one `queue:work` worker as an NSSM service; scheduler via Task Scheduler; no Reverb service, which is Track B), deployed via a **self-hosted GitHub Actions runner** (a green CI run on main → `deploy.ps1`: build the release in a staging worktree while the site stays up, then swap the code and built assets in, migrate, re-cache and `queue:restart` inside a short maintenance window) — see **ADR-0005** (supersedes ADR-0003), as amended by **ADR-0007** (queue workers are a plain `queue:work` process, not Horizon). | Uses hardware the team already owns, so there is no managed-hosting bill; controlling PostgreSQL directly makes Row-Level Security and PostGIS **guaranteed available** rather than a platform capability to verify, and git-driven deploys keep releases simple. Accepted trade-offs: self-managed ops (patching, backups/WAL, pooling) and single-box availability (a single point of failure — see NFR §2). ⚠️ **Amended 2026-09-19 by `M100`: this row said "nginx" without qualification and claimed the box was not yet stood up. The testing site was built on 2026-09-17 and runs Apache; `deploy.ps1` still provisions no Task Scheduler task, so §8 step 7 registers it by hand** (ADR-0007; Doc #22 §8, §8.3). |
...
| R3 | The client (TypeScript) and server (PHP) Expression Evaluator implementations drift behaviorally | A field appears relevant/valid client-side but is rejected server-side (confusing respondent-facing errors), or worse, the client hides a field the server doesn't actually skip, risking silent data loss on submit | A shared golden-file test-vector suite run against both engines in CI; a deliberately small, versioned expression grammar; the server remains sole authority at submission time; client/server evaluation mismatches are logged as telemetry to catch drift in production before it accumulates. **Extended by Doc #26 (piping) — SHIPPED in H6a, COMPLETED in H6b:** answer piping added a **sibling** template grammar (`TEMPLATE_VERSION` `1.0`, `tests/golden/templates/` with **76 vectors**, its own PHP⇄TS runner pair) rather than a mode of the expression grammar, so the expression vectors and `GRAMMAR_VERSION` stay at `2.0` and both existing corpora are byte-untouched. **R3's drift inventory is now THREE corpora**: 180 expression + **114** validation + 76 template vectors (H7 grew the validation corpus by 8 — see the §4.3 note; it is the only increment so far to extend an existing corpus rather than add a new one). The genuinely new drift surface was **not** the hole parser but `SchemaValueFormatter::displayValue()` — H6a pinned its rendering PHP-side (13 `"engines": ["php"]` vectors the TS runner counted but skipped, the first coverage that method ever had), and **H6b built the twin (`engine/display-value.ts`), dropped the `engines` key from the corpus entirely, and grew it 53 → 76**. Building the twin proved `displayValue()` was not mirrorable as written: its bare `(string)` cast produced `'1'`/`''` for a bool and `precision=14` floats, neither reproducible in JavaScript, and both reachable through a `calculated` field. Doc #26 amendment **A7** replaced it with a pinned, ini-independent rule implemented identically on both sides; **A8** added the `?string $locale` that makes a piped choice label resolve into the same language as the sentence around it. **H17 added a FOURTH mirror pair that is not a corpus** — `App\Enums\PdfFieldRole::Omitted` must hold exactly the members of `RENDERS_NOTHING` in `resources/public-runtime/lib/schema-mapping.ts`, because the submission PDF claims to show what the respondent actually saw and the guest SPA decides what they saw. It is pinned by a test that PARSES the TypeScript source rather than comparing against a hand-copied list (`tests/Unit/Forms/PdfFieldRoleTest.php`), since a copied list keeps passing after someone edits the `.ts` file. Recorded here because R3's inventory is drift surfaces, not only golden corpora. **H21a added a FIFTH mirror pair that is also not a corpus, and it is BEHAVIOURAL rather than source-parsing** (Doc #27 §9 as amended by **A6**). It shipped a PHP step projection (`App\Support\Forms\StepProjection`, pure and static, consuming the masks a `SemanticResult` already carries) in H21a rather than waiting for H21c, and pinned it against the guest store's `visibleSteps` with a SHARED FIXTURE — `tests/fixtures/step-projection.json`, sixteen cases of `(sections, sequences, repeat topology, field→section map, answers) → expected step keys`, driven by `tests/Unit/Forms/StepProjectionTest.php` and `resources/public-runtime/__tests__/steps.test.ts`, each through its own engine. The prescribed source-parsing device was **rejected as vacuous**: it works for `PdfFieldRole` because it parses actual DATA (`RENDERS_NOTHING`'s members ARE the rule), whereas a projection's rule is control flow, so parsing it would pin a list of predicate NAMES that either side could implement with inverted polarity and still satisfy. A `tests/golden/steps/` corpus stays rejected for its original reason (the language-neutral vector shape cannot carry a section list, per-section sequences, a repeat topology or a field→section map — Doc #26 amendment A4's argument), and the fixture sits OUTSIDE `tests/golden/` carrying no `grammar_version` key, so **the count stays at exactly 296 and the validation total at 114**. **H21c inherited the primitive rather than inventing one, and then removed the drift surface entirely rather than mirroring it a third time.** The Inertia encode page now mounts `createFormRuntime()` itself, so `visibleSteps` has ONE TypeScript implementation serving both channels — this pair stays PHP-vs-TS and does not become a triangle. The consequence for testing is recorded in Doc #27 amendment C4: a PHP suite that ran both projections and compared them would assert a tautology about one function, so `EncodeStepParityTest` (the fixture’s third consumer, all sixteen cases as real published forms) asserts the INPUT instead — the encode payload’s `version.schema` byte-identical to `PublicFormPresenter`’s, and the taken path rebuilt from that shipped snapshot. The owed cross-check against `SubmissionPdfPresenter` ships as `EncodePdfTakenPathTest`, pinning SECTION-level agreement over three distinct taken paths on the submission’s own pinned clock; the PDF presenter is deliberately NOT refactored onto `StepProjection`, because its "No entries." notice and em-dash blanks are distinctions the step model cannot express. One invariant that H21c found unguarded and now pins: `EncodeFormPresenter::OMITTED` must stay a strict SUBSET of `PdfFieldRole::Omitted`. The two answer different questions ("never a manually-entered answer" vs "the respondent sees nothing"), they overlap only by accident of the current lists, and a mutation exploiting that accident reddened nothing at all. **A SIXTH drift surface H21a had to mirror, recorded because it was nearly missed:** `min_instances` enforcement lives in `SemanticValidator::processRepeats()`, which has a byte-for-byte TypeScript twin that the golden runners also reach, so H21a's narrowing of it (bounds are enforced only on a group the respondent's step list would show — Doc #27 §4.3/A4) had to land identically on both sides; a PHP-only fix would have been an R3 break invisible to CI, because every repeat vector in the corpus uses an ordinary `short_text` member. That mirroring moved `RENDERS_NOTHING` from `lib/schema-mapping.ts` down into `engine/field-roles.ts` (the engine must not import from `lib/`, and a second copy would be a third definition); `schema-mapping.ts` re-exports it and `PdfFieldRoleTest`'s regex was repointed. **Two unpinned coincidences Doc #27 §3.2 makes normative:** the two engines agree on a *cyclic* relevance dependency only because their break predicates happen to coincide (PHP compares whole arrays, TS compares key sets) and because their iteration bounds count the same schema set from different sources (PHP counts Eloquent rows, TS counts snapshot entries). If either count is ever filtered, the engines land on opposite phases of the cycle — a silent R3 break with no vector to catch it, since no cyclic vector exists and H21 adds none. **H21a's own bound-exhaustion fix is exactly this hazard realised** (§3.2/A3): the settle loop returned its section and field masks one iteration apart, and the phase the loop halts on is decided by the field count, so the regression test's fixture size is load-bearing rather than incidental. **H21d1 added NO mirror pair, and the two things that look like one are recorded here so nobody goes hunting for the vectors that would pin them.** The builder's logic canvas parses `relevant_expression` client-side with the TypeScript `ExpressionParser` — the same engine, not a second one — and walks the resulting AST for `${key}` chips. That walk resembles `ExpressionParser::referencedKeys()` and is deliberately not its twin: nothing behavioural rests on it (it drives a chip and a sentence), and whether a reference resolves is still decided by `ExpressionValidationGate` at publish, in PHP. The condition DESCRIBER — AST → plain English — has no PHP counterpart at all and never will: prose is not syntax and cannot be fed back to a parser. What WOULD have been a real sixth pair was deliberately not built: `StepGraphInspector`'s three graph notices stayed on the server behind a read-only `GET /forms/{form}/graph` sidecar rather than being ported to TypeScript, because a hand-written mirror of a cycle detector with no corpus standing over it is precisely the drift this row exists to prevent. The cost is a one-save lag between the live rail and the notices, which the canvas shows on screen rather than hiding. **H21d2 added a THIRD KIND of drift surface — a place where expression syntax is CONSTRUCTED rather than parsed — and it is not a mirror pair either.** The structured condition editor writes `relevant_expression` by PRINTING it (`resources/js/components/builder/condition-model.ts`'s `serialize()`, Doc #27 §8 option (a) / amendment E1), which is the first AST→text path in the repository. There is one printer, in one language, and PHP never prints — so there is nothing to mirror. What IS at risk is that the printer emits text only the TypeScript parser accepts, and that is guarded directly: `tests/fixtures/condition-serializer.json` (27 `(condition, text)` cases, outside `tests/golden/`, no `grammar_version` key, so **the count stays at 296 and the validation total at 114**) is driven by the TypeScript round-trip in both directions AND parsed by PHP's `ExpressionParser` in `tests/Unit/Expressions/ConditionSerializerParityTest.php`. The printer also re-parses its own output before returning it, which turns the lexer's length/token/depth budgets into a refusal at the source rather than three constants that would drift from the lexer the day either moved — load-bearing because **nothing validates a `relevant_expression` on write**, so bad printed text would save cleanly and surface only as a publish refusal. The CLASSIFIER moved in the same increment and is now single by construction: `toCondition()` decides which shapes are representable, and the describer renders its sentence from that model, so the rail's prose and the editor's rows cannot disagree about which expressions are safe to touch (Doc #27 amendment E2). |
...
| R5 | ~~Published-version immutability is enforced only at the application layer~~ — **RESOLVED (2026-08-05)** | — | **Resolved by ADR-0013 (Accepted), built in H25.** The schema's first trigger, `form_versions_published_immutable_trg`: `BEFORE UPDATE … FOR EACH ROW`, gated `WHEN (OLD.status IS DISTINCT FROM 'draft')`, raising SQLSTATE 23001. Four properties are load-bearing rather than stylistic. **(1) Deny by default** — it diffs the WHOLE row via `to_jsonb(OLD)` vs `to_jsonb(NEW)` per key against a four-name lifecycle allowlist (`status`, `superseded_at`, `updated_at`, `published_by`), so a column added by a *future* migration is frozen the moment it exists; a hand-written frozen list would have been fail-open, and nothing in CI would have noticed, since `scripts/migration-lint.php` skips alter-only migrations. **(2) The worst statement in the schema is now refused**: `status = 'draft'` on a published row re-opened every `form_sections`/`form_fields`/`form_field_validations` row beneath it, because the draft-child RLS guard keys on precisely that value; only `published → superseded` is legal, and `superseded` is terminal. **(3) `published_by` may be cleared but never re-pointed** — it is `ON DELETE SET NULL` against `users` and PostgreSQL runs that referential action as an ordinary UPDATE through SPI, which bypasses RLS but *not* a trigger, so freezing it outright would have made user hard-deletion and GDPR erasure permanently impossible. **(4) It binds the superuser**, which RLS cannot: `pgsql_privileged` bypasses row-level security and never bypasses a trigger. **Why this did not stay RLS**: a policy sees only OLD in `USING` and only NEW in `WITH CHECK` and no clause can compare them, while a per-column immutability rule *is* that comparison — so `docs/form-versioning-schema-migration.md` §2's "RLS, not a trigger" resolution stands for the three child tables and is narrowed to them (Doc #14 v1.1 §2.1; ADR-0013 §D1 states the boundary: row-scoped ⇒ RLS, OLD-vs-NEW ⇒ trigger, one member today). **Scope is UPDATE only, and the residual is R12 below.** Honest limits, recorded in ADR-0013 §D12: `meridian_app` owns the table, so `DISABLE TRIGGER` and `session_replication_role` are open to it — this is an integrity guard against application bugs and raw queries (R5's own wording is "accidentally mutates"), not a security boundary. One live defect closed in passing: `FormService::updateMetadata()` wrote `title`/`description` to an in-memory `draft_version_id` with no lock or re-read, so a concurrent publish could make an ordinary form-settings save rewrite a published version — now predicated on `status = 'draft'`. |
...
| R9 | Postgres RLS predicate evaluation adds per-query overhead at scale | Query latency regression under heavy multi-tenant load, particularly on high-cardinality tables like `submissions`/`submission_answers` | The RLS predicate is kept deliberately simple (an indexed `tenant_id` equality check only, no complex subqueries in the policy itself); load-tested during Phase 0/1; the isolation-tiering escape hatch (§5, layer 13) is available without a rewrite if a specific tenant's scale genuinely demands a dedicated database. **Still OPEN.** **ADR-0011 (H1e, 2026-08-03) is the first decision to reason about R9 on the *read* side.** H24a is the first feature to issue grouped, date-bucketed aggregates over `submissions` at range under the RLS predicate, and `submissions` carried no time-ordered index, so H24a owned one index addendum. **As built (2026-08-03):** `submissions_analytics_series_idx` on `(tenant_id, submitted_at) INCLUDE (form_id) WHERE status <> 'draft' AND deleted_at IS NULL`, plus H12a's capacity index *widened in place* to `(tenant_id, form_id, submitted_at)` — a strict prefix-superset, so the cap COUNT is unaffected and the net index count is unchanged, which is what keeps this ONE addendum. The `deleted_at IS NULL` clause is what makes an index-only scan possible at all, and its corollary is a standing rule: **no analytics query may use `withTrashed()` on `submissions`**, or the predicate becomes unprovable and the index is silently lost. The index shape is pinned by structural assertions rather than by `EXPLAIN` — `ScopeNodeScaleTest` already established that on a small table the planner tie-breaks unpredictably, so a plan-shape test measures the cost model rather than the property. Two decisions deliberately keep R9's exposure from growing: §D1 declines a `form_events` table, which would have displaced `submissions` as the highest-cardinality table in the system on a single box; and §D7 requires every analytics query to be time-bounded, form-bounded and fixed-cardinality, with materialization as a fallback triggered by a **measured** p95 breach of the `docs/non-functional-requirements.md` §1 dashboard budget — not chosen up front. **ADR-0017 (P2a, 2026-08-14) narrows this row's mitigation clause without closing the row.** "The escape hatch is available without a rewrite if a tenant's scale demands a dedicated database" was the standing answer of last resort; ADR-0017 §D2 replaces *dedicated database* with *isolated schema in the same instance* (the database variant is rejected — login resolves identity pre-tenant, and three RLS policies subquery a neighbouring table, which is inexpressible across databases), and §D2's ⚠️ records that the hatch needs a custom schema manager and a working per-tenant migration runner before it is reachable at all. **The load test this row has always asked for still has not been run**, so R9 stays **OPEN** on its own terms — what changed is that its fallback is now a named topology with known prerequisites rather than an assumption. |
...
| R12 | A published version cannot be EDITED (R5), but it can still be DESTROYED: `form_versions.form_id → forms` and `.tenant_id → tenants` are both `ON DELETE CASCADE`, and PostgreSQL runs referential actions **bypassing RLS** | A hard-delete of one `forms` row silently removes its published versions, their frozen `form_sections`/`form_fields`/`form_field_validations` content **and every submission FK'd to them** — the same loss of historical meaning R5 describes, reached through deletion instead of mutation. `TRUNCATE` bypasses row triggers entirely, so no trigger would catch that path either | **OPEN, and opened deliberately by H25 rather than inherited silently.** H25 is scoped to `UPDATE` because a `BEFORE DELETE` row trigger fires on FK cascades too, so adding one would turn tenant hard-delete and form hard-delete into an error — a behaviour change far outside a hardening PR, and one needing a purge seam no document currently specifies (ADR-0013 §D2). What holds today is **application posture, not enforcement**: `Form` uses `SoftDeletes`, `FormController` exposes `archive()` and no `destroy()`, and `docs/form-versioning-schema-migration.md` §9 records that archiving discards only the draft. The `formVersionGuard` DELETE policy (`status = 'draft'`) still blocks *direct* deletion of a non-draft version on every tenant connection — the hole is one level up, at the parent. A feature test asserts the cascade behaviour as it stands, so the gap is executable rather than only written down. **Revisit trigger: the first production hard-delete path** — a tenant purge, a GDPR account-deletion tool, or a retention reaper. At that point the DELETE half and its purge seam must be designed, not deferred. Until then the guarantee published everywhere is worded "a published version cannot be edited", never "cannot be destroyed". ⚠️ **P2b (2026-08-14) does NOT fire this trigger, and the distinction is worth stating because it looks like it might.** `tenants:extract` is the READ half of an offboarding; the write half — the purge — is still unbuilt, so nobody should read the existence of an extract command as evidence that a tenant can be removed. ADR-0018 §D1's no-HTTP-surface decision also means there is no route through which a purge could accidentally be reached. When the purge IS built, this row's DELETE half and its purge seam are its first design questions, and ADR-0018's manifest is where "what did we hold" gets answered before "what did we remove". |
...
5. **A database-level guard is recommended, in addition to application-layer enforcement**, to make published-version immutability defense-in-depth rather than single-layer (§6.2). ~~Still open; owned by increment H25.~~ — **CLOSED 2026-08-05 by H25 → ADR-0013** (promoted to a formal ADR, exactly as this appendix's closing line anticipated). Built as `form_versions_published_immutable_trg`: `BEFORE UPDATE … FOR EACH ROW`, gated on the row not being a draft, deny-by-default over a whole-row `to_jsonb` diff, `published → superseded` the only legal transition. **UPDATE only** — the delete half is deliberately not built and is now risk **R12**.
