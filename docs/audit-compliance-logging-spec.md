...
| `form_collaborators` | `created` (access granted), `deleted` (access revoked) | Per-form access changes (`docs/multi-tenancy-rbac-design.md` §8) — a Form Editor/Reviewer gaining or losing access to a specific form is exactly the kind of access-control change an audit trail exists to cover. |
