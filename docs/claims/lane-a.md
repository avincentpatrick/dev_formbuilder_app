...
## Status: ACTIVE CLAIM — `M109`, four `early-testing` rows: the deploy that breaks an open tab, the builder's scrollbar, and two documents that describe the wrong behaviour (`m109-deploy-chunks-and-the-spill`)

Taken 2026-09-25. Branch `m109-deploy-chunks-and-the-spill`, cut from `origin/main` at `4994850`, PR into main.

Rows, all `early-testing`, the most urgent tier with open work:
- `R-e7d6f223` — *"A deploy deletes the previous build's chunks, so an open tab can fail to load a script it needs."* `docs/feature-backlog.md:9937`.
- `R-45b0cf8a` — *"`MdsSegmentedControl` spills 30px INSIDE the builder's config pane — a real horizontal scrollbar."* `docs/feature-backlog.md:630`.
- `R-6c429d4e` — *"`D28`'s chosen option is a no-op as written, and the row that awaits it would implement the illustration."* `docs/feature-backlog.md:11090`.
- `R-2e2407e2` — *"`docs/ACCESS-MATRIX.md:105` tells its reader the central host answers a workspace route with a 404, and it does not — it redirects."* `docs/feature-backlog.md:10857`.

⛔ **TAKEN UNDER `D13` EXCEPTION #2, AND IT IS THE SAME REASON AS #1.** Clause 1 — *no two rows may cite the same non-hub file* — is satisfied **exactly**: the four non-hub sets are pairwise disjoint (`deploy.ps1` and `tests/Feature/Deploy/`; the four segmented-control hosts; nothing; `CentralHostFallbackTest.php`). Clause 2 is breached because all four touch `docs/feature-backlog.md` (degree 9) and `docs/pipeline.md` (10) **by close-out procedure alone**, which is the condition `M108` measured as unsatisfiable by any batch, including a batch of one. ⚠️ **A third exception for this reason is the trigger the log names: at that point the entry to write is an answer to `D15`, not a fourth exception.** `D15` is open, sits in this same tier, and is neither re-asked nor re-scoped here.

### Evidence verified

- **`R-2e2407e2` — every citation holds, exactly.** `docs/ACCESS-MATRIX.md:105` still carries the false sentence; `bootstrap/app.php:613-621` renders `NotASubdomainException` for a **web** request as `redirect(config('app.url'))` and returns 404 only on the `/api/v1` arm; `tests/Feature/Tenancy/CentralHostFallbackTest.php:25` and `:32` both pin `assertRedirect`. Four for four.
- **`R-45b0cf8a` — every CSS claim holds; two citations have MOVED.** `.mds-segmented` is `inline-flex` (`SegmentedControl.vue:78`) with no `flex-wrap` anywhere in the file and `min-width: 0` on the fieldset (`:79`); `.mds-segmented__seg` (`:101-118`) carries no `min-width`, no `flex-shrink` and no `white-space`, at 24px of padding per segment. ⚠️ The Requiredness control is at `ConfigPanel.vue:314-322`, cited `307-312`; `.config` is at `:557-564`, cited `546-553`. Both are repaired at the row.
- **`R-6c429d4e` — holds.** `D28`'s chosen option sits at `docs/claims/decisions.md:1621-1624` and its illustration at `:1622`; the cost table records `flex-shrink: 1` as inert at `:1602`, which is the clue the row says the recommendation fails to carry through.
- **`R-e7d6f223` — the defect holds and the stated MECHANISM is wrong.** There is no grace period of any kind. But the server does not empty `public/build`: `deploy.ps1` builds in `.deploy-stage` (`:376`, `:381`) and swaps by **rename** (`:428`, `:430`), then deletes `public/build.prev` at `:485` and defensively at `:401-403`. `emptyOutDir` — Vite's default, since `vite.config.ts` declares no `build` key at all — is only the local story.

### Premise verified

- ⛔ **`R-2e2407e2`'s premise is FALSE, and the row understates itself.** It records that `M105` *"corrected the sibling copy in `R-5ecfa6cd`'s own headline"*, which is true and is not the whole of it: the **body** sentence at `docs/feature-backlog.md:9586` still reads *"so the central host answers 404"*, contradicted four lines later by its own `M96` correction at `:9590`. **Two authored copies, not one.** A third, byte-identical, sits in `.kilo/worktrees/obsidian-dogwood/` — the untracked second checkout already filed as its own row, and not this increment's to fix.
- ⛔ **`R-e7d6f223`'s premise is INCOMPLETE in the direction that matters.** It frames the residual as *"Inertia's version check reloads a tab on its next navigation"*. That is right — the version is `hash_file('xxh128', public/build/manifest.json)` (`HandleInertiaRequests.php:29-32`), checked only on an Inertia **GET** (`Middleware.php:145`, `:149`). But **the guest runtime is not Inertia at all** (`resources/public-runtime/main.ts` is a plain `createApp`), so it has **no version check of any kind**, and it mounts the same lazy chain through `FieldControl.vue:74` and `InstanceField.vue:13`. The row's *"counted, not exercised"* caveat is accurate and the count is exactly three: `FieldInput.vue:28`, `FieldInput.vue:31`, `GeoInput.vue:302`.
- ⚠️ **`R-e7d6f223` carries one premise that CANNOT be checked from the tree, and it is named here rather than assumed.** `deploy.ps1:29` says the live `.env` is copied into the stage because the build reads `ASSET_URL`. If production sets it, Inertia's version is a constant hash of that string and the navigation reload never fires either. `config/app.php` declares no `asset_url` key and `.env.example` does not set it; the production `.env` is off-repo.
- ⛔ **`R-6c429d4e`'s premise holds, and the entry it corrects carries a SECOND defect the row does not name.** `D28:1600` reads *"4 of 4 … `.mds-modal__body` for the other three"*. `.mds-field` twice and `.sheets-fields` are inside `Modal.vue:453-456`'s `.mds-modal__body` — **three call sites but only two host classes** — while `.encode-field` renders inside an `MdsCard` (`Encode.vue:1221`) and in the public runtime (`FieldControl.vue:74`), **not in a modal**. The line mixes a class count with a call-site count.
- **`R-45b0cf8a`'s premise holds, independently recounted.** 13 call sites across 9 files; **four** stretch-clamped host classes over five of them — `.config__group`, `.mds-field` (twice), `.sheets-fields`, `.encode-field` — every one `display: flex; flex-direction: column` with no `align-items` override. The other nine are flex rows or blocks.

### Remedy verdict

- **`R-e7d6f223` — the row offers two, and the first is taken on the user's answer.** *Keep the previous release's chunks for a grace period* works and costs the user nothing. *Catch a failed chunk load and reload* also works but lands a reload on a respondent mid-form, and ⛔ **would not even see one of the three sites**: `GeoInput.vue:297-328` swallows its own import failure deliberately, so no `vite:preloadError` listener reaches it. Retention needs no client change and no such exception.
- ⛔ **`R-45b0cf8a` — the remedy implied by `D28` AS WRITTEN MEASURES IDENTICALLY, which is why `R-6c429d4e` is in this batch.** `min-width: 0; max-width: 100%` on a stretched child of a **column** flex container changes nothing, and `.encode-field:533` already carries `min-width: 0` and still spills — the counter-example is already in the tree. The affordance that works is `flex-wrap: wrap` on the hosted control, declared at each of the four hosts: `D28`'s chosen option 3, guard-at-the-host, with a working illustration.
- ⛔ **`R-45b0cf8a` — the obvious instrument CANNOT SEE IT, by design.** `assertNoHorizontalOverflow()` (`tests/e2e/support/axe.ts:152-346`) skips any subtree inside an `overflow-x: auto|scroll` ancestor and files it as `absorbed`; `axe.ts:183-191` already names this exact control as that case. `DataTable.test.ts:24-28` is not an alternative — it fabricates `scrollWidth` and `clientWidth`, so happy-dom decides nothing here. The remedy is proved by an element-level Playwright assertion on `.config`, which is what `D28:1630-1634` asks for by name.
- **`R-6c429d4e` and `R-2e2407e2` — text corrections, and both are exact.** The `D28` answer survives; only its illustration and one cost-table line change. The two false-404 sentences are edited in place.

Files: `deploy.ps1`, `tests/Feature/Deploy/BuildRetentionTest.php` (new), `resources/js/components/builder/ConfigPanel.vue`, `packages/design-system/src/components/FormField/FormField.vue`, `resources/js/components/integrations/SheetsRuleFields.vue`, `resources/js/components/submissions/FieldInput.vue`, `tests/e2e/personalization-axe.spec.ts`, `docs/claims/decisions.md`, `docs/ACCESS-MATRIX.md`, `docs/feature-backlog.md`, `docs/pipeline.md`, `PROGRESS.md`, `docs/claims/lane-a.md`, `docs/gate-baselines.md`.
Shared artefacts taken: `docs/ACCESS-MATRIX.md`, `docs/claims/decisions.md`, `docs/feature-backlog.md`, `docs/pipeline.md`, `docs/gate-baselines.md`, `tests/e2e/personalization-axe.spec.ts`, and `PROGRESS.md` (own block only).
Paired files taken: `packages/design-system/src/components/FormField/FormField.vue` — scanned by `scripts/component-import-lint.php` (its own merge-blocking `ci.yml` step) and by `clipped-node-containment.test.ts` and `token-references.test.ts`, all three of which read `resources/js` and `packages/design-system/src` together. `SegmentedControl.vue` is deliberately NOT edited.
Namespaces spent: nothing from either namespace — no migration, no ADR, no sub-decision.
Prediction: Pint passes untouched, since this diff adds no PHP outside one test file. PHPStan does not move and **the honest statement is that its input is byte-identical** — a diff restricted to `app`, `database` and `routes` returns nothing at all. `citation-liveness-lint` stays at 17 rotten against its ceiling of 18, because every document edit is in place and line-count-neutral; that is the tightest gate here, and one unbalanced hunk reddens it. The new Pest case reddens under `mutate.php` in two places — deleting the carry-forward, and inverting its ordering assertion. The `.config` assertion fails on the unfixed tree and passes after the four host rules.

⛔ **THE ONE I MOST EXPECT TO BE WRONG: the `.config` assertion's measured spill.** The 30px in the row is recorded nowhere — `D28:1603` calls it *"unprovenanced — no test, fixture or snapshot records it"* — so the figure this increment measures is the first one with provenance and it may differ. ⚠️ **The failure mode I am actually guarding against is worse than a wrong number.** `.config` is a scroll container only because `overflow-y: auto` forces `overflow-x` to compute to `auto`; if the pane does not overflow at the test viewport, the assertion reads zero on a genuinely broken layout and passes vacuously. The non-vacuity guard therefore has to be on the **measurement**, not only on the element count — which is the same defect class as the `KNOWN_OVERFLOWING` set that `M19` emptied.
