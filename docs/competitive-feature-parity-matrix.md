...
| Conditional step skipping (a whole page/section disappears) | ✓ (section-level `relevant`) | ✓ | ◐ (per-field only in legacy) | ✓ — **already shipped**: a section-level `relevant_expression` removes the step from both the sequence *and* the total, so the respondent sees "Step 4 of 5" recomputed as if the skipped section had never existed for them, rather than a grayed-out placeholder that leaks the form's full logic surface | 1 → 3 | #27 |
| Visual workflow / branching canvas | ◐ (`relevant` is a text formula in a spreadsheet cell — powerful, no canvas) | ✓ (the headline authoring feature) | ✗ | ◐ → ✓ with H21d. **The honest qualifier: this product's branching is a predicate per node, not an author-drawn edge** — there is no "jump to section 7", so a skip over four sections is four conditions rather than one arrow, and the canvas draws a rail with conditions rather than a flow chart. What it buys in exchange is that branching, skip logic and validation all run through **one** expression engine with a dual PHP/TypeScript golden corpus, so a branch cannot mean one thing to the browser and another to the server | 3 | #27 |
| Disqualification / screen-out with a terminal submission state | ✗ | ✓ | ✗ | ◐ — the *mechanism* ships with H21 (relevance can empty the remaining step graph, and Doc #27 §4.1 specifies the terminal screen that follows); the *state* (`screened_out`, with its capacity-count, export and webhook knock-ons) is deliberately deferred, so a screened-out respondent still submits an ordinary record | 3+ | `docs/feature-backlog.md` |
