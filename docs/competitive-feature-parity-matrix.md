...
| Visual workflow / branching canvas | ◐ (`relevant` is a text formula in a spreadsheet cell — powerful, no canvas) | ✓ (the headline authoring feature) | ✗ | ✓ → **SHIPPED — read half H21d1, edit half H21d2.** The builder's centre pane toggles `Structure ⇄ Logic`; the Logic view draws the whole form as a rail with each node's condition, its graph notices and its syntax errors, and selecting a node opens a **structured condition editor** in the config panel — rows of *question · check · value*, nestable `All of`/`Any of` groups, a dropdown of the referenced field's own choices, and an entry-count row for repeat sections. No expression syntax is typed unless the author wants to. **The honest qualifier stands and is now stated on the surface itself: this product's branching is a predicate per node, not an author-drawn edge** — there is no "jump to section 7", so a skip over four sections is four conditions rather than one arrow, and the canvas draws a rail with conditions rather than a flow chart. What it buys in exchange is that branching, skip logic and validation all run through **one** expression engine with a dual PHP/TypeScript golden corpus, so a branch cannot mean one thing to the browser and another to the server | 3 | #27 |
| A condition written as a formula is read back in plain English | ✗ (the `relevant` cell is the only representation) | n/a (there is no formula to read back — the canvas IS the representation) | ✗ | ✓ — **SHIPPED H21d1**: every node shows its expression verbatim AND, where the shape can be expressed in full, a sentence beside it ("Shown when *Your age* is more than 18"). **Anything it cannot express in full shows the raw text alone** — never a partial or paraphrased reading, because a confident wrong sentence about a form that decides who is asked what is worse than no sentence. That same classifier is what H21d2's editor uses to decide which conditions it may touch — and in H21d2 it became **one** classifier rather than two agreeing ones: the sentence is now rendered FROM the structured model the editor edits, so the reading on the rail and the rows in the panel cannot disagree about the same text. The cost is stated rather than hidden — the model has to carry every quirk of the readable set, including comparisons written backwards and field-against-field ones, because narrowing the editor would have silently narrowed the reading | 3 | #27 §8 |
...
| Saved / custom report views per user | ✗ | ✓ | ✗ | ✓ — **shipped H24a**; ADR-0011 §D8 pins the table as strict-RLS with a `user_id` and stores a *declaration* resolved at read time, so a view naming a question that no longer exists degrades to a stated refusal rather than a wrong number. Private to its author, including from an Owner | 3 | #4 |
| Respondent funnel / per-step drop-off / time-on-step | ✗ | ✓ | ✗ | ✗ — **deliberately deferred to Phase 4** (ADR-0011 §D1). The platform records outcomes, not behaviour: a respondent who abandons a form leaves no server-side row, so no denominator exists. Closing the gap means an engagement event stream, a fourth unauthenticated ingress class and the highest-write-volume table in the system on a single box — none of which the Phase-3 acceptance criteria ask for. Two caveats are recorded for whoever builds it: offline respondents would emit no events, and the step graph is relevance-filtered, so "step 3" is not the same node for two respondents | 4 | #4 |
| Charted analytics (trend lines, category breakdowns) | ◐ (basic summary charts) | ✓ | ✗ | ✓ — planned H24b, built **in-house** as design-system SVG primitives over `--mds-chart-*` tokens rather than by adding a charting library (ADR-0011 §D10): the token pipeline emits `var()` references and the theme flips live, so SVG re-paints for free where a canvas library goes stale, and a design-system component is under the merge-blocking axe gate by construction. Capped at five categorical series, with the sixth-and-beyond folded into a neutral "Other (N)" bucket | 3 | #4 / #19 |
