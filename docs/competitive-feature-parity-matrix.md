...
| Saved / custom report views per user | ✗ | ✓ | ✗ | ✓ — planned H24a; ADR-0011 §D8 pins the table as strict-RLS with a `user_id` and stores a *declaration* resolved at read time, so a view naming a question that no longer exists degrades to a stated refusal rather than a wrong number | 3 | #4 |
