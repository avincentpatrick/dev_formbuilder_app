...
> **Indexes:** nothing on `search_vector`, deliberately — for the RLS/leakproof reason given under `forms` **and** for a second, independent one: the submissions arm matches through a three-branch OR whose `form_id IN (<subquery>)` branch is a `SubPlan`, and PostgreSQL abandons index paths for an entire OR clause when any branch yields none. Rescuing the first reason would not restore an index here. See `2026_08_11_000002` and `SearchIndexUsageTest`.
