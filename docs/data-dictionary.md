...
| `guest_token` | `varchar(128)` | Yes | `NULL` | No | Opaque signed session identifier for `source = 'guest'` submissions; correlates the token minted per `forms`' stateless guest-access scheme. |
