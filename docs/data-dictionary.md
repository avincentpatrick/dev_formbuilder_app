...
> - **`owner_user_id` is attribution, not the sole access gate.** Who may actually edit or review this form is governed by the RBAC doc's `form_collaborators` table (external — see RBAC doc, `docs/multi-tenancy-rbac-design.md` §8): Owner/Admin tenant roles get tenant-wide access regardless of this column, while a Form Editor/Reviewer needs an explicit `form_collaborators` row scoped to this specific form. `owner_user_id` remains the form's primary point of contact and the default dashboard-scoping assignee, and its creator is expected to also receive a `form_collaborators` editor row at creation time — but it no longer means "the only person who can edit."
...
>   - Publish integrity is enforced by `StructuralValidationGate` (choice options non-empty + distinct; cascading levels non-empty, every level populated, no dangling parent). `matrix`/`likert_matrix` (object-valued grids) are Increment G4b.
> - **Semantic (Stage-3) error slugs added in G4a** (relevance-aware, run only on answered fields; skipped when a field declares no options, keeping pre-G4a behaviour unchanged): `choice_not_in_options` (single/multi/dropdown/likert_scale value not in `config.options`), `cascading_choice_invalid` + `cascading_parent_mismatch` (per-level, addressed `field.<levelIndex>` via the new `SemanticError.cellPath` sub-axis).
...
> - `accent_token` is validated against the Design System's curated whitelist at the **application layer**, not via a DB `CHECK` constraint enumerating design-system tokens — a `CHECK` would create an awkward cross-document coupling requiring a schema migration every time the design system adds a new accent option.
...
form_collaborators.tenant_id           -> tenants.id         (external table — see RBAC doc, docs/multi-tenancy-rbac-design.md §8)
form_collaborators.form_id             -> forms.id
form_collaborators.user_id             -> users.id           (external)
form_collaborators.added_by            -> users.id           (external, nullable)
