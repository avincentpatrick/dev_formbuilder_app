...
| `subject_email` | `varchar(255)` | Yes | `NULL` | **Yes** | ⚠️ **NULL for every PRE-validation refusal, and that is the rule rather than an omission.** An address only exists once a signature over the assertion carrying it has verified; recording one from a document that failed validation would let anyone on the internet write chosen text into a tenant's database and onto an admin's screen. Populated only by `jit_disabled`, `existing_account_not_member`, `membership_suspended`, `seat_quota_exhausted`, `step_up_unknown_subject` and `step_up_subject_mismatch`. ⚠️ **`existing_account_not_member` WAS MISSING FROM THIS LIST FROM M1 UNTIL M2**, which is the hazard the rule itself describes: a list of exceptions is only as good as the discipline of extending it, and the one reason most likely to mean somebody is probing for an account was the one the document did not say recorded an address. **M2's `idp_certificate_unusable` is on the NULL side and belongs there for a reason worth stating**: unlike every other pre-validation refusal its assertion may well carry a perfectly valid signature over a real address — but that signature has not been checked when the refusal fires, so the address is still attacker-chosen text. |
