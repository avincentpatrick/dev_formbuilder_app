...
⚠️ **What the `Default` column below means, stated once because it was wrong for a long time:** it records **where the value comes from**, and a **function name in it means a real database-side `DEFAULT` that Postgres will apply**. Exactly two columns in this schema have one — `audits.created_at` and `feedback_reports.submitted_at`, both from `->useCurrent()`. Everywhere else the value is supplied by the application: application-generated (`HasUuidv7`) for primary keys, `set by Eloquent` for `created_at`/`updated_at`. `tests/Feature/Migrations/DocumentedDefaultDriftTest.php` holds this column to the live schema, so a function name that no longer matches a real default fails the build rather than misleading a reader.
