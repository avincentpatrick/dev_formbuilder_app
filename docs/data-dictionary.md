...
**Primary key strategy:** every table's `id` is a `uuid`, and **the value is generated by the application before the row is ever sent to Postgres** — `App\Models\Concerns\HasUuidv7` (Ramsey's `Uuid::uuid7()`), used by 45 models. **No table carries a database-side UUID default, and none is wanted.** UUIDv7 was chosen over `uuidv4`/`gen_random_uuid()` for better B-tree index locality (time-ordered inserts avoid the random-insert page-split churn that pure v4 causes at scale), and over auto-increment `bigint` because: (a) IDs are exposed in the public API and public share links, where sequential integers leak cross-tenant record counts and are trivially enumerable; (b) the offline PWA client (plan §2.4) must be able to generate a valid, collision-safe primary key **before** it ever reaches the server, which auto-increment cannot do. ⚠️ **Reason (b) is also why generation is client-side rather than database-side, and it is independent of the server version.** Postgres grew a native `uuidv7()` in **PostgreSQL 18**; this deployment runs **PostgreSQL 17**, so the function does not exist here — but even on 18+ the offline client must mint the key first, so a DB-side default would never be the thing that filled the column. Adopting the native function later would be an optimization for server-side inserts, not a correction. **Five tables deliberately deviate and use `bigint identity` instead** — `submission_answer_index`, `usage_counters`, `legacy_overrides`, `point_awards` and `badge_awards` — because they are pure internal aggregation/projection rows, never addressed externally, generated only server-side in high volume, where a narrower key measurably helps index size at scale; this is called out again in each table's own Design Notes. (`domains` is a third-party table from stancl/tenancy and keeps that package's `integer` key — see §1a.)

⚠️ **What the `Default` column below means, stated once because it was wrong for a long time:** it records **where the value comes from**, and a **function name in it means a real database-side `DEFAULT` that Postgres will apply**. Exactly two columns in this schema carry a *function-shaped* one — `audits.created_at` and `feedback_reports.submitted_at`, both from `->useCurrent()`. ⛔ **CORRECTED 2026-09-07 (M83): this paragraph used to continue "everywhere else the value is supplied by the application", and that was false by a factor of fifty.** **102 columns carry a real database-side default**, and the literal cells below record them — 41 of 86 comparable pairs match the live schema byte-for-byte with no normalization at all. A **literal** in this column therefore means the same thing a function name does: Postgres will apply it. What is genuinely application-supplied says so in words, from a closed vocabulary — `application-generated (HasUuidv7)` for primary keys, `set by Eloquent` for `created_at`/`updated_at`, `NULL`, `—`, `auto-increment`, `identity`, `*derived*`. `tests/Feature/Migrations/DocumentedDefaultDriftTest.php` holds all three shapes to the live schema: function names by **presence** (`now()` and `CURRENT_TIMESTAMP` are one default written two ways), literals by **equality** after stripping the Postgres cast, the quotes and JSON whitespace, and any prose it does not recognise fails the build rather than being guessed at.
...
**Enum & vocabulary strategy (stated once, per plan §5 "one consistent enum strategy"):** the default for every fixed, code-controlled vocabulary in this schema is a **native PHP 8.1+ backed enum**, stored as a plain `varchar` column — **often, but NOT always, with a Postgres `CHECK` constraint mirroring the enum's cases; see the `DB CHECK` column of the catalog below for which, corrected by `M87`** — and cast to the PHP enum via Eloquent's `casts()` — this fixes legacy's mixture of magic-number columns, lookup tables with no Eloquent model, and stray free-text columns living next to FK-backed siblings. Every such column is marked in its table with **"PHP enum: `EnumName`"** in the Type cell, and the full value list is documented once below rather than repeated in every table. This document introduces **no new lookup tables** for vocabulary beyond what the plan itself already specifies (`field_library`/`form_templates` are reusable *content*, not vocabulary; `plans` is the platform's *pricing catalog*, not a vocabulary lookup, even though it plays a similar structural role to one — see its Design Notes) — every closed vocabulary in the schema fits comfortably in an enum, and legacy's own lookup tables that had "no Eloquent model" (`form_submission_statuses`, `submission_methods`) are exactly the pattern being eliminated. One deliberate, explicitly-flagged exception exists: `subscriptions.stripe_status` is stored as free text, not a PHP enum, because it mirrors Stripe's own status vocabulary verbatim as synced by Cashier — Stripe controls and can extend that vocabulary outside this app's release cycle, so constraining it locally would risk silently rejecting a legitimate future Stripe status. This is called out again in that table's Design Notes.

**PHP backed enum catalog** (all values in `snake_case`). ⛔ **THE `DB CHECK` COLUMN IS THE CORRECTION `M87` (2026-09-08) MADE, AND THE HEADLINE IS THAT THIS PREAMBLE USED TO CLAIM THE OPPOSITE.** It read *"matching each enum's Postgres `CHECK` constraint"*, as a universal, and **17 of the 28 rows below have no `CHECK` at all** — so a reader was told that seventeen vocabularies are database-enforced when they are enforced only by the PHP cast. Every cell in that column is measured from `pg_constraint` on the live schema rather than grepped from `database/migrations/`, and `tests/Feature/Docs/DocumentedCheckConstraintDriftTest.php` now re-measures it on every run. ⚠️ **The census that produced "3 of 16" was wrong in both numbers and the reason is worth keeping**: it grepped `database/migrations/`, and one CHECK — `form_versions_status_chk` — is created by first-party code in `app/Support/Migrations/PublishedVersionGuard.php`, outside that directory. Not by its `_chk` suffix, which seven constraints inside the directory also carry: **by its location**.

| Enum | Backing column(s) | DB CHECK | Values |
|---|---|---|---|
| `TenantStatus` | `tenants.status` | — none | `active`, `suspended` — **corrected 2026-09-07 (M83)**; this entry listed `trial`, `active`, `suspended`, `cancelled` against an enum that declares exactly two cases, and `trial` was also the column's documented default. ⚠️ **`tenants.status` has no `CHECK` constraint**, in the live database or in any migration, so the preamble's universal above does not hold here. |
| `FormStatus` | `forms.status` | — none | `draft`, `published`, `archived` |
| `FormVersionStatus` | `form_versions.status` | `form_versions_status_chk` | `draft`, `published`, `superseded` |
...
| `AttachmentKind` | `attachments.kind` | — none | `submission_file`, `ocr_source_scan`, `field_media_sample`, `signature_capture`, `avatar`, `branding_logo`, `feedback_screenshot` *(added I7a)*, `export_artifact`, `webhook_payload_archive` |
...
|---|---|---|---|---|---|
| `id` | `uuid` | No | application-generated (`HasUuidv7`) | No | Primary key; also the value stamped into the RLS session variable and into every queued job payload (plan §2.1). |
...
| `status` | `varchar(20)` — PHP enum: `TenantStatus` | No | `'active'` | No | Lifecycle state of the whole account. **Corrected 2026-09-07 (M83): this cell read `'trial'`**, which the create-`tenants` migration contradicts (`->default('active')`) and which `TenantStatus` cannot even represent — the enum declares two cases, `Active` and `Suspended`. It was the single live drift the literal arm of `DocumentedDefaultDriftTest` found across 86 comparable pairs. No CHECK constraint exists on this column; the enum is enforced in PHP only. |
...
| `is_tax_exempt` | `boolean` | Yes | `NULL` | No | ⚠️ **This column does not exist — corrected 2026-09-07 (M83).** The row documented a `false` default and a Stripe Tax behaviour; it appears in no migration and is absent from `Tenant::getCustomColumns()`, which is the authoritative list. `docs/pricing-feature-gating-matrix.md` reasons from it as a built column — that is the same shape as the `data_residency_region` row below, where an argument elsewhere rested on a reservation that was never made. Tax exemption needs the migration first. |
| `timezone` | `varchar(64)` | Yes | `NULL` | No | ⚠️ **This column does not exist — corrected 2026-09-07 (M83).** The row documented a `'UTC'` default; it is in no migration and not in `Tenant::getCustomColumns()`. **The real column with that name and that default is on `forms` (§2, H12a)**, which is why the claim survived so long. `docs/non-functional-requirements.md` calls this one an "already-modeled column"; it is not modeled at all. Tenant-level timezone needs a migration, not a row here. |
...
| `settings` | `jsonb` | Yes | `NULL` | No | ⚠️ **This column does not exist — corrected 2026-09-07 (M83).** The row documented a `'{}'` catch-all for tenant configuration. There is no such column in any migration and none in `Tenant::getCustomColumns()`. Tenant-level configuration that is not its own column lives in the separate **`settings` table** (§20), which is a different thing with a different key space — a second catch-all on `tenants` was never built and would be the two-sources-of-truth defect §20's Design Notes argue against. |
...
| Column | Type | Nullable | Default | PII? | Description |
|---|---|---|---|---|---|
| `id` | `uuid` | No | application-generated (`HasUuidv7`) | No | Primary key; this is the value every `submissions.form_version_id` FKs to. |
...
| Column | Type | Nullable | Default | PII? | Description |
|---|---|---|---|---|---|
| `id` | `uuid` | No | application-generated (`HasUuidv7`) | No | Server-assigned primary key — see Design Notes for why this is distinct from `client_submission_uuid`. |
| `tenant_id` | `uuid` | No | — | No | FK to `tenants.id`. |
...
| `event` | `varchar(30)` — PHP enum: `AuditEvent` | No | — | No | See the **eleven-value** catalog above, pinned in the database by `audits_event_check`. *(Read "8-value" until Increment M46, 2026-08-29 — the two impersonation boundaries were added to the enum and the constraint in I11b and never reached this reference; "10-value" until M107 widened it for `two_factor_reset`.)* |
...
> **Design Notes**
> - `event` extends legacy's 4 base cases (`created`/`updated`/`deleted`/`restored`) with **seven** domain-specific events (`published`, `archived`, `exported`, `permission_changed`, — **AS BUILT (I11b), corrected here by Increment M46 (2026-08-29)** — `impersonation_started` and `impersonation_ended`, and **`two_factor_reset` (M107, `D37`)**) that the plan's versioning, export, RBAC, platform-operator and account-recovery model need tracked explicitly — a noted, reasonable extension, not a literal 1:1 port. **Eleven cases in total, and the database pins all eleven via `audits_event_check`.** ⚠️ This bullet read "4 domain-specific events" from I11b until M46; a reader of a data dictionary has no way to notice a count that is two short, which is exactly why the catalog is the thing that has to be right.
...
| `key` | `varchar(100)` | No | — | No | Dot-namespaced key. **As built**: `registration.invite_only` (I5) and `security.require_two_factor` (I8a) are tenant-only; `registration.open_signup` / `maintenance.enabled` / `maintenance.message` are platform-only (I5); plus the open `modules.<plan_feature_key>` namespace. ⚠️ **The side is a property of the KEY, not of the caller** — `App\Enums\SettingKey::scope()` decides it, and because both readers filter on `tenant_id`, a key written to the wrong side is *invisible* rather than wrong. `tests/Feature/Docs/DocumentedSettingKeyDriftTest.php` holds this list against the enum, so a sixth key cannot land without appearing here. |
...
> 2. **The table is SPARSE, and the defaults live in PHP.** An absent row means "default", never null: `App\Enums\SettingKey::default()` is the single place each one is written down (`registration.invite_only` ⇒ true, `security.require_two_factor` ⇒ false, `registration.open_signup` ⇒ true, `maintenance.enabled` ⇒ false, `maintenance.message` ⇒ the empty string), and `modules.*` defaults to *enabled* in `TenantSettingRegistry::moduleEnabled()`. Every read path returns a RESOLVED value, so no surface null-coalesces and two surfaces cannot pick different fallbacks. ⚠️ **`security.require_two_factor` defaults FALSE, and that is deliberately not the fail-closed reading** — the reason is argued in that method's own docblock and is not restated here: the table is sparse, so "absent" is the state of every workspace that already exists. Do not "correct" it to true.
...
| Column | Type | Nullable | Default | PII? | Description |
|---|---|---|---|---|---|
| `id` | `uuid` | No | application-generated (`HasUuidv7`) | No | Primary key. Not addressable by any route — the resource is a singleton at `/settings/sso`. |
| `tenant_id` | `uuid` | No | — | No | FK to `tenants.id`, `ON DELETE CASCADE`. |
...
| Column | Type | Nullable | Default | PII? | Description |
|---|---|---|---|---|---|
| `id` | `uuid` | No | application-generated (`HasUuidv7`) | No | Primary key. Time-ordered, which is what makes the trim's tiebreak total. |
| `tenant_id` | `uuid` | No | — | No | FK to `tenants.id`, `ON DELETE CASCADE`. |
...
| `created_at` / `updated_at` | `timestamptz` | No | set by Eloquent | No | `updated_at` never moves — no UPDATE policy exists. |
