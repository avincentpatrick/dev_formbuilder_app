...
| `event` | `varchar(30)` — PHP enum: `AuditEvent` | No | — | No | See the **ten-value** catalog above, pinned in the database by `audits_event_check`. *(Read "8-value" until Increment M46, 2026-08-29 — the two impersonation boundaries were added to the enum and the constraint in I11b and never reached this reference.)* |
...
> **Design Notes**
> - `event` extends legacy's 4 base cases (`created`/`updated`/`deleted`/`restored`) with **six** domain-specific events (`published`, `archived`, `exported`, `permission_changed`, and — **AS BUILT (I11b), corrected here by Increment M46 (2026-08-29)** — `impersonation_started` and `impersonation_ended`) that the plan's versioning, export, RBAC and platform-operator model need tracked explicitly — a noted, reasonable extension, not a literal 1:1 port. **Ten cases in total, and the database pins all ten via `audits_event_check`.** ⚠️ This bullet read "4 domain-specific events" from I11b until M46; a reader of a data dictionary has no way to notice a count that is two short, which is exactly why the catalog is the thing that has to be right.
...
> - **AS BUILT (I11a) — the two actor columns, and why the direction is not arbitrary.** `user_id` keeps meaning "the account this happened under" and `acting_as_user_id` is the addition. Every existing consumer — `audits_tenant_user_idx`, the viewer's actor filter, `AuditPolicy`, the export's Actor column, `AuditResource.user_id` — already reads `user_id` with that meaning, so swapping the sense would silently re-point all of them at platform staff. A reader that knows nothing about impersonation therefore keeps telling the truth, just less of it. ⛔ **ONE CHECK PAIRS THE TWO — `audits_acting_as_not_self_check`, `CHECK (acting_as_user_id IS NULL OR acting_as_user_id <> user_id)`, added by `2026_08_09_000001_add_acting_as_user_id_to_audits.php` and dropped in its `down()`.** It encodes I11's "never yourself" decision of record at the layer that cannot be bypassed: without it, an `impersonator_id` left in the session after exit writes a row saying a user impersonated themselves — not merely noise, but a ledger entry that reads as an accusation. `app/Enums/AuditEvent.php` notes that the two impersonation cases satisfy it for free. ⚠️ **This paragraph asserted "No CHECK pairs the two" from I11a until Increment M46 (2026-08-29), and the reasoning underneath it was sound but attached to the wrong constraint.** The migration considered two and the document transcribed the **rejected** one: `CHECK (acting_as_user_id IS NULL OR user_id IS NOT NULL)` really would have refused a row the retention policy produces, because `user_id` is nullable for system actions and both FKs are `nullOnDelete`, so deleting the impersonated user legitimately leaves a NULL `user_id` beside a non-NULL `acting_as_user_id`. The constraint that **shipped** does not have that problem and the migration says why in its own comment: it passes on NULL in either column, since `NULL <> x` is NULL and a CHECK passes on NULL, so it never fights the FKs. **The cost of the error was concrete: a backfill or fixture setting `acting_as_user_id = user_id` gets a 23514 from a constraint the canonical schema reference denies exists.** **The tenant page renders only "Platform operator", by POLICY** — `actingAsLabel()` never reads the name, and the relation is not eager-loaded. (The original justification, that RLS hides staff anyway, is false: an operator who also holds an active membership of the impersonated tenant is visible, and the first draft leaked their name.) **`/admin/audit-log` cannot display these rows at all** — they are tenant-scoped and that viewer reads only the `tenant_id IS NULL` slice — so `/api/v1`'s opaque uuid is currently the only surface that identifies the operator.
> - **The second CHECK on this table is `audits_event_check`**, created with the table from `AuditEvent::values()` and widened for the two impersonation boundaries by `2026_08_10_000002_recreate_audits_event_check_for_impersonation.php`. ⚠️ **Named here by Increment M46 because §13 enumerates CHECK constraints elsewhere, so its absence read as "there is no such constraint".** It is generated from the enum exactly once, at the migration that creates it — **adding a case to the PHP enum does nothing to a database that already exists**, which is why widening it needed its own migration rather than an enum edit.
