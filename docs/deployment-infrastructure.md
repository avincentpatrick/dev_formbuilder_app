...
7. **Scheduler** — a Windows Task Scheduler task running `php artisan schedule:run` every minute (Windows has no cron). **Gap: `deploy.ps1` provisions no such task, and the application-side schedule does not exist yet** — `routes/console.php` is stock until H2 builds the `Schedule::job(...)` declarations (ADR-0007). Until both halves exist, nothing periodic runs in production and nothing drains the queue.
