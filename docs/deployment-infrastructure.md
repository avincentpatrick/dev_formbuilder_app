...
2. **Action**: the workflow invokes **`deploy.ps1`** (committed at the repo root) against the live application directory. `deploy.ps1`:
   ...
   - restart the Horizon and Reverb Windows services (so workers/realtime run the new code) + `horizon:terminate` for a graceful worker cycle,
   - `php artisan up`.
...
- **Queue priorities** (Horizon), reflecting the six submission channels and other async work: `submissions` and `webhooks` highest (user-facing / near-real-time); `exports` and `ocr-processing` medium (a pending state is shown); `scheduled-maintenance` (usage rollups, retention purges) lowest.
- **Process supervision**: Horizon runs as a **Windows service via NSSM** (`php artisan horizon`), auto-restarted on crash. Scaling is **manual** on a single box (more Horizon processes / a bigger box) — there is no managed autoscaling; capacity is a deliberate operational choice, and a scale-out need is one of ADR-0005's revisit triggers.
- **Per-tenant fairness** (closes `docs/architecture/technical-architecture.md` Risk R6): jobs are tagged with `tenant_id` (required on every job payload, ADR-0002 §D3); a per-tenant job-rate ceiling prevents one tenant's bulk OCR/export burst from starving other tenants' queued work.
...
6. **Horizon & Reverb** — `php artisan horizon` and `php artisan reverb:start`, each a Windows service via NSSM.
7. **Scheduler** — a Windows Task Scheduler task running `php artisan schedule:run` every minute (Windows has no cron).
