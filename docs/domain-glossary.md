...
| **Capability class** | A role's tenant-wide standing to *potentially* hold a given permission (e.g., Form Editor "can, in principle, edit forms") as distinct from actually holding access to a *specific* resource instance, which requires a `form_collaborators` row — `docs/multi-tenancy-rbac-design.md` §8. |
