...
| **Taken path** | *(no equivalent term)* | *(no equivalent term)* | Re-derived by `App\Services\Submissions\SubmissionPdfPresenter` via `SemanticValidator::validate(..., $now)`; field-type visibility is `App\Enums\PdfFieldRole` | The subset of a form one respondent was ACTUALLY SHOWN — every question their answers made relevant, and no other. Distinct from "the form" (every question an author wrote) and from "the answers" (what they typed): a question shown and left blank is on the taken path, and one hidden by relevance is not, which is a difference no other read surface in this system draws. It is not stored anywhere — `SemanticResult`'s masks are computed at submit and discarded, and a pruned key is indistinguishable from an answered-blank one — so it is re-derived with the clock pinned to the submission's own finalisation time (Increment H17). |
