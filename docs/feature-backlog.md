...
| Disqualification / screen-out (early-exit) logic + a `screened_out` submission state | should | 3 (with the visual workflow builder) | Expression engine can hide/validate/compute but cannot terminate a form early |
...
| Logic testing / preview / logic-map tool | nice | 2–3 | Authoring-confidence for Persona A's complex forms |
