...
Publish (§3.2), version-restore (§6), archive, every STRUCTURAL builder edit, the schedule sweeper's open and close passes, XLSForm import, the submit-time capacity check and a resource grant whose target is a form all acquire a row lock on the owning `forms` row (`SELECT ... FOR UPDATE` on `forms.id` at the start of the transaction) for their full duration. This serializes concurrent attempts at any of them against the *same* form — e.g., two editors both clicking "Publish" within the same second resolve to one succeeding and the second either blocking briefly then reading the now-updated state, or erroring cleanly, rather than racing to double-reserve a `version_number` or double-clone a draft. This does not serialize ordinary field-level edits from different collaborators (§8 covers that separately, at finer grain) — only these form-level lifecycle transitions. ⛔ **CORRECTED BY `M89`, WHICH FOUND THIS PARAGRAPH WRONG IN BOTH DIRECTIONS.** It named four operations; the tree has NINE call sites, listed above. And two of the four it named were not among them: **form creation takes no lock at all** — correctly, since it CREATES the `forms` row — and **"draft-discard" names a method that exists nowhere in `app/`**, the nearest thing being `FormService::archive()`. ⛔ **AND THE LOCK NAMED HERE DOES NOT REACH THE CHILD ROWS.** A publish freezes `form_sections`, `form_fields` and `form_field_validations` into a snapshot it never locked, so a lock-free content edit — which the sentence above deliberately permits — could commit inside that window and leave the frozen `schema_snapshot` and `checksum` describing rows that had already moved. RLS refuses nothing there: under READ COMMITTED the writer's `EXISTS (... status = 'draft')` is evaluated per statement against the committed snapshot, which says `draft` until the publish commits. `M89` locks those three tables at the TOP of the publish transaction, before the validation gates, and `tests/Feature/Forms/PublishLockingTest.php` pins both the placement and the fact that no lock may be taken after the status flip.
...
- **Optimistic concurrency, not locking**: each `form_sections`/`form_fields`/`form_field_validations` row's existing `updated_at` timestamp (already present on every table per the Data Dictionary's timestamp convention — no new column needed) serves as the concurrency token. A client's edit request includes the `updated_at` value it last read; if the server's current `updated_at` for that row has since moved, the write is rejected with a `409 Conflict` and the client re-fetches before retrying — the same "409 for a genuine conflict" pattern the architecture plan already applies to offline-sync replay (§2.4), reused here rather than inventing a second conflict-resolution idiom. ⛔ **`M91` ADDED A ROW LOCK ON THIS PATH AND THIS SENTENCE STILL STANDS, WHICH IS WORTH STATING BECAUSE THE TWO READ AS CONTRADICTING.** `FormBuilderService::writeField()` takes a transaction-scoped `FOR UPDATE` on the single `form_fields` row it is about to write, before it touches any `form_field_validations` row. **It arbitrates nothing.** The concurrency token above is still the only thing that decides whether an edit wins or loses, `assertNoDrift()` still answers `409` before the transaction opens, and two editors on different rows still never conflict. What the lock fixes is ACQUISITION ORDER: Eloquent skips a clean `save()`, so a resubmitted identical payload used to reach the validation rows without having touched the field row, which is the reverse of the order a publish takes them in (§3.4) — a genuine `40P01` deadlock that surfaced as an unrendered 500. `tests/Feature/Forms/BuilderLockOrderTest.php` pins the order on both the clean and dirty paths. ⚠️ A SECOND cycle, reachable only through a cross-field validation rule naming a sibling field, is **not** closed by it and is filed in `docs/feature-backlog.md`.
