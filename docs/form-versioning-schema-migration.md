...
|---|---|---|
| **Non-breaking** (recorded, never warned) | Label/hint/placeholder text changes; reordering fields or sections (`sequence`); adding a new optional field or section; adding a non-required validation rule; widening a constraint (e.g., increasing `max_length`); moving a field between sections (`form_section_id` change) | Fields/sections keep full cross-version comparability under their shared `key`. |
...
`docs/multi-tenancy-rbac-design.md` §8 permits multiple Form Editors to collaborate on the same form via `form_collaborators`, which makes concurrent draft editing a real scenario this doc must address (not merely a hypothetical):
