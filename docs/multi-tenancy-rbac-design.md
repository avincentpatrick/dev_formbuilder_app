...
| **Reviewer** *(a.k.a. Validator in PRD prose)* | `docs/data-dictionary.md` §7 (`submissions.validated_by`): *"reviewer who moved status to approved/returned"*; §7's `remarks` column: *"Internal reviewer notes"* | Review submissions (approve/return, write `remarks`) **on forms they are a collaborator on** (§8), export (`submissions.export`) submissions on those same forms, and view that form's dashboard. Cannot edit form structure, cannot manage users/settings. ⛔ **NOT encoding, and this cell said otherwise until M77.** The role holds `submissions.create`, but `SubmissionPolicy::create()` has required `forms.edit.any` or **editor** capacity since G10a (see §8.3's deliberate-semantic-change note), and a reviewer's grant is reviewer capacity — so a plain Reviewer can manual-encode on **no form at all**. Encoding needs a second, editor-capacity grant on top of the role. |
...
| `submissions.create` *(G10a — `SubmissionPolicy::create()` additionally requires `forms.edit.own`-style **editor** capacity, exactly as the `submissions.edit.own` row below records; a Reviewer's ✓ here is therefore the coarse half only, and confers no encoding without a second editor-capacity grant)* | ✓ | ✓ | ✓ (own forms) | ✓ (coarse only — see note) | |
...
> both can reach the form hub, a form's responses and a submission. (⛔ **M77 — this sentence used to end
> "and the encode screen", and that was false for BOTH roles**: a Viewer holds no `submissions.create` at
> all, and a Reviewer's grant is reviewer capacity where `SubmissionPolicy::create()` requires editor
> capacity. The breadcrumb defect it describes is real and unaffected — the crumb is reached from the
> pages that ARE listed.) A hard `href:
...
| `welcomed_at` | `timestamptz` | Yes | `NULL` | No | **When this person was greeted, once, ever (M103).** `SendWelcomeEmail` claims it with a conditional `UPDATE … WHERE welcomed_at IS NULL`, so `Verified` firing again — which it does on EVERY address change, because `UpdateUserProfileInformation` nulls `email_verified_at` and re-sends — sends no second welcome. ⚠️ **`email_verified_at` could not be the guard**: it is nulled by the very action that starts the repeat. Written under a BORROWED user GUC (`app.current_user_id` set to the person's own id for one transaction), because the ambient GUC is absent on the Google sign-in door and an unqualified UPDATE would silently affect zero rows. Backfilled from `email_verified_at` by `2026_08_17_000113` — including for invitees, who are force-verified and deliberately never welcomed. **Withheld from tenant extracts** ({@see TenantExtractColumns}), in `tos_accepted_at`'s class. |
...
`SubmissionPolicy`'s review check follows the identical shape against `submissions.review.own` — but ⛔ **it is capacity-INSENSITIVE, and this sentence said `capacity = 'reviewer'` until M77.** `review()` calls `collaboratesWith()`, which is `ResourceGrantResolver::holdsAny()`, and `holdsAny()` loops `ResourceCapacity::cases()` and accepts **either** capacity. That is deliberate and load-bearing: it is why the one configuration this section's own note implies — a Reviewer who also holds an editor grant, so they may both review and encode — is possible at all. Under the sentence this replaces it would not be: `capacity` is deliberately **outside** `resource_grants_target_user_unique` (one capacity per target, per this section), and `ResourceGrantService::grant()` REPLACES any existing grant on that target rather than adding a second — so nobody can hold reviewer **and** editor capacity on one form. A capacity-sensitive `review()` would therefore make reviewing and encoding mutually exclusive on every form, which is not what G10a decided. This two-layer composition — a coarse Spatie permission ("can this role ever do this action, in principle") plus a fine Policy-layer resource check ("does a grant cover *this specific* resource") — **is** the general pattern for per-instance scoping beyond the tenant level. Increment G10a made it literal: adding a third scopeable type is one `ResourceScopeable` case plus one migration, never a `switch`.
