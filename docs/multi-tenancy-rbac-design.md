...
**Open questions — flagged, not decided here** (consistent with this project's established practice of naming an unresolved decision explicitly rather than silently picking one, e.g. PRD Feature #11's guest-feedback scope note):

1. **Impersonation.** Does the platform console support "log in as this user" for support purposes? If yes, `audits` likely needs an `acting_as_user_id`-style column to distinguish "the super-admin did X" from "the super-admin did X while impersonating user Y" — not designed here since it isn't yet confirmed to be needed.
2. **Audit-log visibility of super-admin actions.** Should a tenant's own Audit Log (PRD Feature #12) surface actions the platform support team took against that tenant (transparency), or hide them (support confidentiality)? Genuinely unresolved.
3. **Internal staff graduation.** Is platform-side access purely binary (`is_super_admin` true/false), or does the support team itself need graduated tiers (e.g., read-only support vs. full super-admin)? Nothing in the PRD addresses the platform's own internal staffing model.
