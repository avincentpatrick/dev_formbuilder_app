...
| Planned-maintenance windows | Allowed, announced ≥ 24h in advance via the Settings-doc `maintenance.enabled`/`maintenance.message` mechanism (`docs/data-dictionary.md` §20) | Short per-deploy windows (`artisan down`/`up` around the code swap, migrate and the caches, with the release built before the window opens; measured at 7.9 seconds on the Windows testing server in its first automatic deploy, 2026-09-17, with no migrations pending, none when the commit is already live, and none when only paths the site does not load have changed since it — `docs/`, `tests/`, `scripts/`, `.github/` and four top-level markdown files, denied by default) are the norm on the single self-hosted Windows box; true zero-downtime is a future enhancement (atomic release-swap) or a host change — see ADR-0005 / Doc #22 §3.1 |
