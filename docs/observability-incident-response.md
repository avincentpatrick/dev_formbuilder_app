...
| Queues (Horizon) | Per-queue depth and processing rate for each priority tier `docs/deployment-infrastructure.md` §6 defines (`submissions`, `webhooks`, `exports`, `ocr-processing`, `scheduled-maintenance`). |
...
| Queue depth runaway | Any queue's depth exceeds a sustained-growth threshold (queue is filling faster than it's draining) for 10 minutes | High |
