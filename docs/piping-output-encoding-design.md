...
**The one permitted raw-HTML sink in the *front-end* codebase** is `resources/js/components/settings/TwoFactorSetup.vue:110`, whose `v-html` renders a Fortify-generated 2FA QR SVG. It is the sole `v-html`/`innerHTML` in `resources/js`, it is server-generated and same-origin, and **it must never be given form content**.
