...
| **excluded** (10) | `matrix`, `likert_matrix` | Object-valued (`{row:{col:cell}}` / `{row:score}`); already banned as expression operands by `ExpressionValidationGate`. One rule now governs both reference kinds. ⚠️ **M74:** `displayValue()` now has real arms for both (`q1: c1=v1; q2: c1=v3`), so the "it would render machine noise" half of this reasoning is spent. The exclusion stands on the other half — a two-dimensional answer flattened into the middle of a sentence is not a sentence. |
...
| | `file_upload`, `image_capture`, `audio_capture`, `video_capture`, `signature` | The stored answer is a list of attachment-reference envelopes. ⚠️ **M74 corrected this cell's mechanism and then spent it.** Such an answer never reached the `json_encode` *scalar* fallback — being a list, it hit the `is_array` branch first and was `json_encode`d **per envelope** — and `displayValue()` now names the files (`scan.png; back.png`). The exclusion stands on the remaining ground: a filename is not an answer to the question a hole asks, and piping one would imply the file itself is reachable from the sentence. |
...
| Blade shells (`app.blade.php`, `public-runtime.blade.php`) | HTML text + attribute | `{{ }}` only — **on these two views it genuinely is `htmlspecialchars(ENT_QUOTES)`, because they do not render through `Markdown::render()`**; attribute values follow `app.blade.php`'s **whitelist** precedent rather than blind interpolation | — (holds) | existing header/shell tests |
...
| Queued mail (H3/H23) · **text** context | Markdown → HTML | Enable `Markdown::withSecuredEncoding()`. Script execution is currently blocked only *incidentally*, by `EncodedHtmlString`'s `htmlspecialchars`; secured encoding is what neutralises **markdown syntax** | ✅ **CLOSED by H23a4** — `AppServiceProvider::boot()` | Pinned by `BrandedMailRenderTest`: a tenant named `[Reset your password](https://evil.example)` renders as literal text, no anchor and no `<img>`. **Residual, stated rather than implied:** `*`, `_` and backticks stay live, so an adversarial name can still render italic or as a code span. That is typography; both vectors this row names need `[`. |
| Queued mail (M57) · **attribute** context | Markdown → HTML attribute | ⛔ **`App\Support\Mail\MailAttribute::escape()`. A Blade echo is NOT an escaper on this surface** — see §5.2. | ✅ **CLOSED by M57** — `resources/views/vendor/mail/html/header.blade.php` | `BrandedMailRenderTest`: a tenant named `Acme" onerror="alert(1)` yields an `<img>` whose attribute set is exactly `src, class, alt, style` and whose `alt` holds the whole payload as a value. Enforced by `scripts/mail-attribute-lint.php`, a merge-blocking CI step. |
...
**The one permitted raw-HTML sink in the *front-end* codebase** is the `.tfa__qr` element of `resources/js/components/settings/TwoFactorSetup.vue`, whose `v-html` renders a Fortify-generated 2FA QR SVG. (Named by its element rather than by line number since `M98`: the line had already moved twice, and a bare `:N` is a citation no gate can check.) It is the sole `v-html`/`innerHTML` in `resources/js`, it is server-generated and same-origin, and **it must never be given form content**.

⛔ **THIS PARAGRAPH USED TO SAY "IN THE ENTIRE CODEBASE" AND WAS FALSE WHEN IT WAS WRITTEN, IN A WAY WORTH KEEPING (M57).** It scoped its own evidence to `v-html`/`innerHTML` in `resources/`, then generalised the conclusion to every raw sink everywhere — and the published mail header was carrying two `{!!` the whole time. **A negative claim is only as wide as the search that produced it**, and this one silently widened between its clause and its conclusion. It is also why the rule below is now stated in terms of *context* rather than *directive*.
...
### 5.2 A Blade echo is not an escaper on the markdown-mail surface

⛔ **`Markdown::withSecuredEncoding()` REMOVES QUOTE ESCAPING FROM EVERY BLADE ECHO IN A MAIL VIEW, AND THAT IS NOT A SIDE EFFECT — IT IS WHAT THE CONTROL DOES.** `Illuminate\Mail\Markdown::render()` replaces `EncodedHtmlString`'s encoder for the duration of the render with a **three-character map**: `[`, `<`, `>`. **No `"`. No `'`. No `&`.** Off this surface `{{ }}` really is `htmlspecialchars(ENT_QUOTES)`; on it, `{{ }}` and `{!! !!}` are **identical**.

Three consequences, each measured rather than reasoned:

- **`{!!` is not the hazard and counting it gates nothing.** A gate keyed on the raw-output directive would have been fully green against the defect that produced this section. `scripts/mail-attribute-lint.php` keys on *a Blade echo inside a quoted attribute* instead, and one of its four controls is the same defect written as `{{ }}`.
- **A quote break-out is not repaired downstream.** `CssToInlineStyles` re-parses and re-serialises the whole document, so it is tempting to assume the DOM round-trip sanitises. It does the opposite: `alt="Acme" onerror="alert(1)"` is *normalised into* a second, live attribute, because the break-out happened at string concatenation before any parser saw the markup. `&` is the only character the round-trip does fix on its own.
- **Escaping again is wrong twice over unless `double_encode` is off.** The value has already been through the secured map, so `&lt;` at the default becomes `&amp;lt;` — shipped to exactly the images-off audience an `alt` exists for. `App\Support\Mail\MailAttribute::escape()` is that map's missing half and nothing more; its docblock carries the mechanism.

**Text context in a mail view stays raw and must not be "fixed".** The map has already escaped `<` and `>`, so no tag can be opened, and re-escaping would double-encode. The published header's no-logo branch is the live example.
