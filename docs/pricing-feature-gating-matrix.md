...
**⚠️ Added by K1a (2026-08-17) — and it is the ONE row in this table that gates nothing.** `gamification` is granted on **every tier including Free**, by a product decision of record: it is one of the six standing principles (PRD §3.7), and a tier ladder would make the principle invisible to precisely the tenants a first-run experience exists for. It appears here, and in `PlanCatalog::FEATURE_KEYS` at all, for one mechanical reason — `ToggleableModules::KEYS` may contain nothing the plan catalog does not (`SettingsVocabularyTest` pins it), and the tenant's own Settings → Modules switch is the only control anyone has over the engine. Read the row as "universal", not as "sold at every tier": nothing about it is a commercial lever, and a future decision to make it one would be a change to ADR-0020 §D6, not merely to this table.
