...
- **Expression-evaluator golden-file suite**: a shared test-vector set consumed by both the PHP (server-authoritative) and TypeScript (client UX) engines, treating any drift between them as a production defect (`docs/architecture/technical-architecture.md` §4.3, Risk R3).
  - **Template golden-file suite (Doc #26 §7, from H6a)**: piping ships a **sibling** grammar, not a new mode of the expression grammar, so it gets its own vector set (`tests/golden/templates/`) with its own `template_version` manifest and its own PHP⇄TS runner **beside** the two expression suites — which stay pinned at `grammar_version` `2.0`, untouched. Same drift-is-a-defect rule, same Risk R3. Coverage must include value *formatting*, not only hole parsing: `SchemaValueFormatter::displayValue()` has no TypeScript twin today, so H6b builds one and every formatting choice in it (choice-label resolution, `yes_no`, multi-select joining, decimal rendering) becomes a new drift surface. `Coercion::toStr()` is explicitly **not** the primitive for a piped `decimal`/`calculated` value — its non-integral float→string parity is documented as unpinned.
