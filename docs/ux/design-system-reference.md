...
>
> ⚠️⚠️ **AMENDED (J6): THE PHASE IS THE FEATURE, AND IT WAS ALSO A PAGE-GLOBAL ESCAPE SINK. THE FIX IS A PRIORITY RULE, AND IT IS THE FOURTH FACT THIS FAMILY NEEDED.** A capture listener on `document` runs before **every** Escape claimant on the page, including ones this component has no relationship with. The paragraph above reasons only about a dialog *around* the tooltip, and is right about that case; it is written as though the tooltip were always in the reader's interaction context. **Measured reproduction:** at 481–1024px, rest the pointer on a collapsed sidebar rail item while the **account menu** is open and press Escape — the bubble vanished and the **menu stayed open**. The application's shared dismissal composable binds Escape on `document` in the *bubble* phase, so it never ran at all; `MdsMenu`'s root handler and `MdsModal`'s panel handler lose the key the same way.
>
