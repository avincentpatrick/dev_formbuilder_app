...
> ⚠️ **`MdsDataTable` did NOT gain a row-link prop in J2a, and that is a decision rather than an omission.** The J2 plan carried an opt-in `rowHref` so a row could link to its object. Building it was refused on two grounds, recorded here so it is not re-proposed as an oversight. **(1) Every one of these tables ships `#row-actions` containing buttons**, and an anchor wrapping a row would put interactive content inside interactive content — axe's `nested-interactive`, and a real defect for anyone navigating by links. The only safe implementation is therefore "link the first cell", which **(2) `#cell-<key>` already does**, in `forms/Index.vue` and elsewhere, and has since before J2. A second mechanism for one job is how a component comes to disagree with itself; the shipped one stays the single way. **A row-level link is only reconsiderable if a table with no row actions ever needs one**, and it would still be the first cell that carried it.
...
> **As-built note (J2a).** The component decides link-ness from the crumb's **position**, not from whether it carries an `href`: a caller who supplies one for every crumb still gets a correct trail, because a self-link at the tail is both a 2.4.4 nuisance and the thing that makes the trail's end indistinguishable from its body. `Breadcrumb.test.ts` pins that direction specifically, since a `v-if="item.href"` implementation passes every other assertion in the file. It mounts through `PageHeader`'s `#breadcrumbs` slot, which had existed with **zero consumers** since the shell was written. **Six pages hand-rolled a single `← Back to X` link with their own scoped CSS before this** (`forms/Analytics.vue`, `forms/Builder.vue`, `submissions/Show.vue`, `submissions/Encode.vue`, `webhooks/Show.vue`, `integrations/RuleShow.vue`) — twice over the exceptions-log's own "three-plus deviations signal a missing shared component" threshold.

**Tab navigation — `MdsTabNav`** *(**AS-BUILT since J2a**)*: the strip that moves a reader between the pages of **one resource** (a form's Overview / Submissions / Builder / Analytics / Share). Underline-style indicator (`--mds-color-action-primary-bg`, the semantic token for §3.4's original `--mds-primary-600`), 2px on the active item, with a heavier weight and `aria-current="page"` as the two non-colour channels.
...
>
> So when **any** datum carries an `href` the plot stops being one image: `role="img"` and its `aria-label` come off, and the summary moves into a visually-hidden paragraph immediately before the plot so §D12's one-sentence overview is not lost. The tracks stay `aria-hidden` either way, so nothing gains "a dozen unlabelled images". With no hrefs the output is unchanged, and `BarChart.test.ts` asserts **both** directions — an `isInteractive` that collapsed to a constant would otherwise silently strip `role="img"` from every chart already shipped. **The value stays outside the anchor**: the link goes to the category, not to its count.
>
> **`MdsStatTile` gained `href` in the same increment**, as a link and never a click emit — the `MdsCard` `interactive`/`as` precedent, and §3.5's standing rule that an interactive card is a real `<button>`/`<a>`. Optional, so the eleven pre-existing tiles keep rendering a plain container rather than gaining eleven tab stops.
