...
**Four binding notes on the search input (added J1a).**
...
4. **`resources/js/components/builder/LibraryPicker.vue` predates this row and does NOT satisfy it.** It renders a raw `<input type="search">` with its own geometry (34px min-height, `--mds-radius-sm`, `--mds-space-2` padding, `body-md` type) rather than `MdsTextInput`. Adding this row makes that a §1.3-loggable deviation, so it is named here instead of being left for a reader to discover a contradiction: **it is filed for migration in J1e**, which is the increment that touches the list-page keyword filters. Until then it is the one search field in the product this row does not describe.
...
> **Implementation status (J1a): SPECIFIED HERE, NOT YET BUILT.** §3.4.1 below is written *before* the code, deliberately (the I12 precedent — a spec the increment consumes rather than invents). J1a ships only the design-system primitives it needs (`MdsTextInput type="search"`, `MdsModal`'s `initialFocus`); **J1b builds the nav centre region and J1d the palette.** Until then `TopNav.vue` still has two flex children and its own header comment still defers global search to increment **C3** — that comment is stale in both directions (C3 closed without building it, and it is a J1 deliverable), and **J1b corrects it**.
>
