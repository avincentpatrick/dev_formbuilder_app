...
>
> **Chart tokens — status.** The `--mds-chart-*` family does **not** exist yet. ADR-0011 §D11 specifies its contract (see §4.1 below); H24b emits it. Until then, no chart token may be cited as available — an earlier claim elsewhere in the corpus that chart tokens already existed was false and has been corrected.
...
### 3.11 Data Visualization *(specified 2026-08-03 by ADR-0011 §D10–§D12; **not yet built** — H24b emits these)*
