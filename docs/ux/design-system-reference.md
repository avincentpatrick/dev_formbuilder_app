...
> ⛔ **ANY ROLE IN THE DISPLAY STACK NEEDS AN `overflow-wrap` ESCAPE, AND THE REASON IS NOT AESTHETIC (added M19, 2026-08-26).** The Display stack's first three families do not exist on Linux, so `system-ui` decides it — **Segoe UI Variable Display on Windows, DejaVu Sans on a CI runner, ~27% wider for the same string** (measured: 256px against 324px for the word "Submissions" at 48px/750). A display-size role is therefore the one place where a single unbreakable word can be wider than any column you sized for it, and where the difference is invisible to whoever wrote the CSS. **`overflow-wrap: anywhere`, not `break-word`**: only `anywhere` reduces min-content, and these roles are almost always flex items whose automatic minimum size *is* their min-content. ⚠️ **`min-width: 0` on the PARENT is the guard that looks like it covers this and does not** — it lets the container shrink, and the text then spills out of the shrunken container instead. Three defects of exactly this shape shipped before the end-to-end gate could see them (`page-header__title`, `builder__title`, `MdsStatTile`'s value); all three were text nobody here could render at the width CI renders it. Reproduce with `docker compose run --rm e2e` before concluding a display-role box fits.
...
> **These are outside the strict ordering above and must never be substituted for a numbered step.

> **⚠️ AS-BUILT (J4b) — ELEVATION AND PAINT ORDER ARE TWO DIFFERENT QUESTIONS, AND THE TOOLTIP IS WHERE THEY DISAGREE.** The table above assigns `--mds-shadow-3` to tooltips and `--mds-shadow-4` to modals, and the ordering rule says a component at elevation *N* never sits beneath one at *N−1* it means to overlay. A tooltip on a control **inside** a dialog must paint above that dialog while wearing `shadow-3`. That is not a contradiction to resolve by promoting it: the shadow ladder classifies **surfaces by their distance from the page**, and a tooltip is not a surface — it is an annotation glued to its trigger, inheriting whatever context the trigger is in. It keeps `shadow-3` and takes a higher layer.
>
> **THE LAYER SCALE, which J4b minted because there was none.** Five rungs lived as hard-coded literals in four files; `packages/design-system/tokens/z-index.json` now names them.
>
> | Token | Value | What sits there |
> |---|---|---|
> | `--mds-z-index-menu` | 30 | Anchored panels in the top nav — the account menu, the notification popover, the feedback trigger |
> | `--mds-z-index-drawer` | 40 | The sidebar's mobile drawer and its scrim |
> | `--mds-z-index-modal` | 1000 | The modal backdrop, **and** the base the inert stack raises each open backdrop from by its depth |
> | `--mds-z-index-tooltip` | 1050 | Above any realistic modal stack, below the toasts |
> | `--mds-z-index-toast` | 1100 | The toast host — highest by design, and the one surface exempt from the inert stack |
>
> ⚠️ **`modal` MUST EQUAL THE INERT STACK'S OWN CONSTANT, AND A VITEST CASE HOLDS THEM TOGETHER**, because a scale that drifts from the code it describes is worse than no scale — it reads as authoritative. ⚠️ **AND EVERY DECLARATION CARRIES A LITERAL FALLBACK — `var(--mds-z-index-tooltip, 1050)` — GUARDING A FAILURE NO GATE HERE CAN SEE.** The tokens ship from a build step whose output is gitignored, so a developer running the dev server without having built them gets an invalid declaration, `z-index` computes to `auto`, and the tooltip paints *under* an open dialog. CI always builds tokens first, which is precisely why CI could never reproduce it.** `shadow-0..5` describe *distance from the page* — a stacking claim, which is why the ordering rule can be stated at all. A glow describes *a fill bleeding into its surroundings*; a glowing button is not "above" a card, it is coloured. Two different questions, two different token families.
>
...
> ⚠️ **The pre-auth pages are a third, documented variant of this rule** (`AuthLayout`, exceptions-log #1),
> and since J3b that variant itself has two forms: a slim ~400px card for the seven utility pages, and a
> split panel on the two front doors (Login, Register) recorded as exceptions-log #14. The split is keyed
> on a **container** query rather than one of §5's breakpoint tokens, for the reason #12 and #13 give:
> a media query's `em` resolves against the root font size, which `[data-font-size]` does not move.
...
**States**:
- **Default** — populated rows, sortable column headers **only on a table handed its whole dataset** (click/keyboard-activatable, with a visible sort-direction glyph — never sort-order-by-color-alone), a filter bar above the table (chips for active filters, a clear-all action), and pagination below. ⚠️ **A server-paginated table declares no `sortable` column and states its fixed order in one line of prose above the table** (user decision 2026-08-18; precedent `audit/Index.vue`): `MdsDataTable` sorts only the rows it was handed, so over one page of many a sort header reorders that page and announces an order the dataset does not have, and `resources/js/Pages/paginated-tables-sort.test.ts` fails a page that pairs the two. **As built, pagination is `MdsPagination`** — previous/next buttons and a "Page X of Y · N results" summary, with no rows-per-page selector — over an **offset** `paginate()` at a page size each presenter fixes; cursor pagination is the `/api/v1` contract, not these pages.
- **Loading** — a **skeleton state**, not a spinner overlay: the table renders its actual column structure with shimmering placeholder blocks matching each cell's approximate content width, so the layout does not jump once real data arrives. **As built, it shows while a filter or page round-trip is in flight** — a page that passes `:loading` holds it for the length of the Inertia visit — and it replaces only the row region, keeping the header and filter bar stable. A client-side sort reorders in place and never re-fetches, and an Inertia page arrives with its first rows already rendered, so there is no first-load skeleton.
...
- ⚠️ **The palette's live region must live INSIDE the dialog.** `inert` removes the background from the accessibility tree, so *every* `aria-live` region outside an open modal stops announcing and nothing replays on close (§4.5's I10a amendment). The result count is announced by a `role="status"` inside the panel. The palette must **not** become a `data-mds-inert-exempt` surface. ⚠️ **AMENDED (J4b): an earlier version of this sentence said the exemption "belongs to the toast host alone", which §3.4a:858 contradicted in the same document by instructing J4b to take a second one — the two-copies-drifted-apart shape, inside a single file.** `MdsTooltip` now holds it too. **The rule the palette clause was really about is narrower and still binding: a LIVE REGION must never become an exempt surface**, because an announcement about content the reader cannot reach is noise carrying authority. A tooltip is `role="tooltip"` rather than a live region, and it is exempt for a different reason entirely — it is anchored to a control that may itself be *inside* the dialog, so inerting the bubble would drop the description of a control the reader is actively using. The prohibition on the palette is unchanged.
...
- The chord path moves focus to the trigger **before** opening, so the modal captures a focusable opener rather than `<body>`. Otherwise closing a keyboard-opened palette strands focus — the outcome §4.5 forbids.
  - ⚠️ **AMENDED (J6) ON A POINT OF FACT: `document.body.focus()` IS NOT A SILENT NO-OP**, which is what this bullet used to say. The body is the document's **default focus target**, so the call succeeds — which inverts the consequence. A captured `<body>` opener does not fail to restore focus, it actively **takes** focus, including out of a dialog still open above. `MdsModal` now refuses to capture one at all. The sentence was believed for two increments and was load-bearing in two docblocks.
  - ⚠️ **AND MOVING FOCUS BEFORE OPENING IS NOT SUFFICIENT ON ITS OWN, WHICH IS WHY §4.5 GAINED `returnFocus`.** It only works if the captured element is still there at close time, and this surface makes that routinely false: activating a result **navigates**, the shell re-renders, and an opener captured inside a since-closed mobile drawer is gone. The same `openerSelectors` list is therefore passed to `MdsModal` as `returnFocus`, so the modal re-resolves against the document *as it stands then* rather than trusting a handle taken before the page changed. **One list for one question** — two lists would drift.

⚠️ **THE CHORD'S GUARD ASKS ABOUT DIALOGS, NOT ABOUT ANYTHING HOLDING THE PAGE (as-built, J6).** ⌘K declines when a blocking dialog already owns the page, and swallows the keystroke while declining — handing ⌘K back to the browser there opens its find bar on top of a modal, which is worse than nothing happening. **A non-dialog surface is not a reason to decline**: the mobile nav drawer (§6) takes the page via the same `inert` stack, and until J6 it was counted as a dialog, which made ⌘K a **dead key** whenever the drawer was open — a global affordance asking *"would I be stacking onto an unfinished dialog?"* and getting *yes* from a navigation flyout. Stack entries now declare their kind and `openModalCount()` counts only dialogs, which is what it had documented since J1a. **The palette stacking over an open drawer is the intended behaviour** (user decision, 2026-08-17), not a tolerated side effect.
...
**Consolidation trigger.** The combobox is hand-rolled in the app tree because the increment that owns the missing primitives (`MdsCombobox`, `MdsTabs`, ~~`MdsMenu`~~ — **built in J4b, see §3.4b** — and an input-adornment wrapper) runs *after* this one, and generalising a palette — whose options are navigation targets, which writes no value back, and which has no persisting selection — would produce the wrong primitive. **When `MdsCombobox` is built, the command palette is its first refactor target.** The `exceptions-log.md` entry is owed by **J1d**, the increment that actually builds the component — an exceptions entry describes a live deviation, and the log's entries all carry an `Introduced:` increment, so filing one against a directory that does not exist yet would make the log lie about its own count.

⚠️ **Coverage gap, recorded here because a green gate will otherwise be misread as coverage** — **now stated generally in §4.6.1, which is what to cite.** Storybook globs `packages/design-system/src/**/*.stories.@(ts|tsx)` only, so an app-tree component gets **no story and no `checkA11y` scan**. ✅ **AMENDED J4c: this no longer describes the whole palette.** The combobox half moved into `MdsCombobox`, which has stories and is scanned; the modal wrapper and the shell around it are still gated by Playwright alone.

> **AS-BUILT (J4c) — `MdsCombobox` EXISTS, AND THE PALETTE IS A CONSUMER OF IT.** The hand-rolled implementation this section described was a logged deviation whose disposition said to retire it *by deletion* when this component landed. That has happened; the exceptions log carries a tombstone rather than an entry, and the number is not reused.
>
> **WHAT MOVED, AND WHAT DELIBERATELY DID NOT.** Into the package: the input's `role="combobox"` and its four ARIA attributes, option id generation, grouped rendering, the active-descendant model, the ↓↑/Home/End/Enter keyboard, and the polite live region. Left in the palette, because none of it is combobox behaviour: the ⌘K chord, the 250ms debounce, the abort-plus-monotonic-sequence fetch discipline, the synthetic "see all" row, the single-string empty copy, and the modal.
>
> **THE OPTION LIST IS FLAT WITH AN OPTIONAL GROUP NAME, NOT NESTED, AND THAT IS A CORRECTNESS CHOICE.** The flat index **is** the `aria-activedescendant` index. A nested structure would carry a second index whose only job is to agree with the first, and a relationship that can disagree with itself is exactly what this component exists to prevent. Consecutive options sharing a name render inside one `role="group"`; an option with no name renders as a **direct child of the listbox**, which is what keeps the synthetic trailing row from acquiring a heading that is nowhere on the screen.
>
> **THE CONSUMER OWNS THE LIVE-REGION WORDING.** The component takes it as a prop and never counts for itself: only the palette knows its last row is synthetic and must not be counted, and §3.4.1's disclosure rule makes zero-result copy one string with one code path — which a component computing its own count would quietly branch.
>
> ⚠️ **IT DOES NOT BIND ESCAPE, AND THAT IS A DECISION RATHER THAN AN OMISSION.** The ARIA pattern would have Escape collapse the listbox first; its consumer sits inside `MdsModal`, whose Escape is the dismissal, so swallowing the first press would make closing the dialog take two. **That makes it the third member of a family that must not be "aligned"**: `MdsTooltip` listens on `document` in the capture phase because it never holds focus (§3.4a), `MdsMenu` binds its own root because it always does (§3.4b), and `MdsCombobox` binds neither because the surface that owns its Escape is the dialog around it. A consumer that mounts it **outside** a dialog owes Escape its own handler, with its own written argument.
>
> ⚠️ **AND THE FAMILY GAINED A GOVERNING RULE IN J6, BECAUSE THREE DIFFERENT BINDING SITES ARE NOT A PRIORITY SCHEME.** Choosing *where* to listen decides who runs first; it does not decide who should **win**, and the two came apart the moment a capture-phase listener met a bubble-phase one it knew nothing about (see §3.4a's J6 amendment — a hovered rail tooltip was eating an Escape aimed at the account menu). **The rule for every member: a component may consume Escape only when the key is aimed at it, which means when it contains `document.activeElement` — or when nothing holds focus, so nothing else can be claiming it. Dismissing yourself is unconditional; consuming the key is not.** `MdsMenu` and `MdsCombobox` satisfy this structurally, because they only listen where they already hold focus; `MdsTooltip` is the one member that has to test for it, because it is the one member that can be visible while the reader is somewhere else entirely.
>
> ⚠️ **THE LISTBOX IS IN FLOW, NOT AN ANCHORED POPUP.** The one consumer renders it inside a dialog body. An anchored variant needs §3.4a's teleport-plus-exemption construction and a placement module, and building it now would ship API with no consumer — the mistake §3.4a records against itself. **Owed by its first real consumer, in that consumer's own PR**, exactly like the `MdsIconButton` title-suppression prop.
>
> ⚠️ **THE EXTRACTION MOVED THE ROOT ELEMENT, AND THAT SILENTLY BROKE THE MODAL'S INITIAL FOCUS — CAUGHT BY THE UNIT SUITE, WHICH IS THE HALF WORTH RECORDING.** `MdsTextInput`'s root element **is** the input under Vue's default `inheritAttrs`, so `data-mds-initial-focus` had been landing on something focusable. `MdsCombobox` has a real wrapper, so the same attribute moved onto a `div` — and `.focus()` on a non-focusable element is a **silent no-op**. §4.5 names that outcome precisely: a keyboard trap (WCAG 2.1.2) reached through the very prop meant to prevent one. It degraded rather than broke, because `MdsModal` verifies focus landed and falls back — but the fallback is the close affordance, not the search field, which is not what a ⌘K user asked for. **The fix is to name the control: the palette mints the input's id and passes `initialFocus` as a selector for it.** ⚠️ **The general rule: an attribute that worked because a component had no wrapper is a dependency on that component's shape, and wrapping is not a refactor-safe operation for it.**
>
> **THE PALETTE'S OWN TEST FILE PASSES BYTE-UNEDITED**, which is the evidence this is an extraction rather than a change — the `Modal.test.ts` precedent from `useInertBackground`. All ten cases, including the six that pin the ARIA contract.
...
> **⚠️ AS-BUILT (J4b) — THE SIDEBAR IS GROUPED, AND THE GROUPING IS A PARTITION OF THE EXISTING ORDER RATHER THAN A REORDERING.** Four runs: an unlabelled lead (Forms, Submissions, Dashboard, Analytics), **Administration** (Members, Scopes, Audit log, Feedback), **Connections** (Webhooks, Integrations, Domains), and an unlabelled tail (Settings). Every item sits exactly where it already sat — `navItems` is now *derived* by flattening the groups, and a test asserts the flat order is unchanged, which is what makes this presentational rather than a re-ordering. The runs were not invented either: three separate comments in `nav-model.ts` had each described the same Administration cluster from a different angle, which is three authors identifying one group and having nowhere to put it.
>
> **TWO LABELS, NOT FOUR, AND THE TWO NULLS ARE THE LOAD-BEARING HALF.** Heading the lead run would put a category name over the four destinations this section already names as the primary ones. Settings is its own tail run rather than the end of *Connections* because all three connection items are plan-gated: an Owner on Free would otherwise read **"Connections → Settings"**, a heading over the one item it does not describe.
>
> **A GROUP WHOSE ITEMS ARE ALL GATED AWAY RENDERS NOTHING** — no label, no empty list, no separator. This is not defensive coding: `visibleItems` ANDs a permission gate with a plan feature, and **the floor is TWO items** (Dashboard and Settings are the only ungated ones, so off-tenant chrome shows just those). A viewer below Business sees three; an Owner on Free loses the whole *Connections* run entirely. A headingless run emits no `aria-labelledby` at all rather than an empty one, so there is never a dangling idref.
>
> **⚠️ THE LABEL IS A `<div>` OUTSIDE THE `<ul>`, WIRED BY `aria-labelledby` ON `<ul role="list">`, AND EACH OF THOSE THREE CHOICES IS PINNED BY A DIFFERENT GATE.** *Outside the list*, because axe's `list` rule is WCAG-tagged and therefore inside the e2e scan's tag set — and the sidebar is scanned on every whole-page `assertClean`, so a heading between a list and its items would redden roughly forty cases at once. *Not a `<span>`*, because `Sidebar.test.ts`'s label helper selects `nav a, nav span` and asserts four item names are ABSENT for gated-away users; a span here joins that text. *Not an `<h2>`*, because this nav renders ahead of every page's own `<h1>`, so headings here would lead each page's outline with shell chrome. `aria-labelledby` announces the group when the reader enters it, which is where the information is wanted, and costs no outline at all.
>
> **AT ≤1024px THE LABEL IS `display: none` AND A 1px RULE TAKES ITS PLACE** — never a visually-hidden duplicate, which is JR4's rule: a clipped control is still a focus stop the user cannot see. Nothing is lost, because §6 and this section both make the glyph the sole signifier at that width, and **the group's accessible NAME survives regardless** — `aria-labelledby` traverses a hidden referent, so the accessibility tree is identical at all three widths. That is what §6's "a page never needs to know which of the three is currently rendering" actually promises.
>
...
>
> **⚠️ AND JR5'S PANE SWITCHER IS NOT THAT STRIP — THE DECISION ABOVE STANDS UNCHANGED.** What was refused was a second **navigation** row: `MdsTabNav`, a landmark of links where each item is a URL with its own route and its own gate, costing ~44px of permanent header on the one screen that cannot spare it. JR5's control is none of those things. It carries no href, no `aria-current` and no navigation landmark; it is a `radiogroup` that changes which of three **already-mounted panes of the same page** occupies the viewport — the category §3.4 calls *"views of the same resource, panels already in the document"*. The governing layout rule is likewise untouched, because this is not a navigation paradigm at all: it is responsive layout, the same category as `MdsDataTable`'s card collapse. **And it is `display: none` above 60em of the builder's own container**, so at the desktop workspace the decision above was about, it costs exactly zero vertical pixels. The vertical-space objection does not transfer, because the row it adds appears only below that threshold — where the alternative it replaced was scrolling past thirty-one palette buttons to reach the canvas, which is vertical space that was already being spent, far more expensively. Exceptions-log **#13** records the derivation of 60em and the five costs. (One factual correction to the paragraph above while it is being read: `.builder` is a **column flex container** since JR5, not `grid-template-rows: auto 1fr` — that rule assumed two children and handed the flexible row to a warnings banner whenever one was up.)
>
...
**Tabs — `MdsTabs`** *(**AS-BUILT since J4c**; J2a built `MdsTabNav` above for the route-navigation case and deliberately did **not** repurpose this spec for it)*: used to switch between views of the *same* resource without a full navigation, where the panels are already in the document. Underline-style indicator (2px on the selected tab — see the token note below, which corrects what this line used to prescribe), full keyboard support (arrow-key roving tabindex per the ARIA Tabs pattern, §4.5). `MdsSegmentedControl` remains the right control for the small in-page case where the thing being switched is *not* a panel the control owns (it is a `radiogroup` — see the builder's Structure ⇄ Logic toggle **and, since JR5, its compact pane switcher**, both of which stay radiogroups permanently per the ⚠️ note below).

> ⚠️ **THE INDICATOR TOKEN IN THE LINE ABOVE WAS WRONG AND HAS BEEN REMOVED — IT SAID `--mds-primary-600`, WHOSE SEMANTIC NAME IS THE `-bg` FILL, AND THAT IS THE THIRD TIME THIS DOCUMENT'S OWN SENTENCE HAS PRODUCED A 1.4.11 FAILURE.** J2a corrected it for `MdsTabNav` two paragraphs above and left this paragraph carrying the original; J4a measured the same substitution on the personalization accent bar at **2.54:1 dark**; and `ConfigPanel.vue` — the only implementation of *this* paragraph — was still drawing its selected underline with `--mds-color-action-primary-bg` when J4c arrived. **The correct token is `--mds-color-action-primary-fg`**, for the reason stated at length in the `MdsTabNav` note: `-bg` is a fill whose only guaranteed contrast is against the text printed *on* it, while `-fg` is paired against both `surface` and `canvas`, in both themes, for every tenant brand. A leftover instruction in a spec is not inert — something eventually implements it.
>
> ⚠️ **AND ON THIS STRIP IT WAS A LIVE FAILURE RATHER THAN HYGIENE, WHICH ONLY MEASUREMENT COULD TELL APART** — J4a's two cases went opposite ways on exactly this question. Measured in the running builder at 1440px across all four shipped combinations, old → new:
>
> | accent × theme | `-bg` (as shipped) | `-fg` (as built) |
> |---|---|---|
> | blueprint · light | 4.71:1 | **7.01:1** |
> | blueprint · dark | 3.42:1 | **8.29:1** |
> | teal · light | 7.48:1 | 7.48:1 — the two tokens resolve to the same colour here |
> | **teal · dark** | **2.41:1 — FAILS the 3:1 a non-text indicator owes** | **6.39:1** |
>
> So every author using the teal accent in dark — a shipped, user-selectable §2.9 preference, not a hypothetical tenant brand — had an underline below the threshold, on the one control that says which config tab they are editing. The other three combinations passed and were merely unmargined. ⚠️ **The four `-fg` values reproduce the JR1 numbers recorded in the `MdsTabNav` note to the digit**, which is independent confirmation that the two strips now draw the same thing rather than two things that happen to look alike.

> **AS-BUILT (J4c). The paragraph above was written before the component; what follows is only what implementing it decided.**
>
> **THE COMPONENT OWNS THE PANEL AS WELL AS THE STRIP, AND THAT IS WHAT MAKES ITS WIRING UNWRITEABLE-WRONG AT A CALL SITE.** A strip-only component would leave the ids, `aria-controls` and `aria-labelledby` split across two files, which is exactly the arrangement that produced a tablist with **no `aria-controls` at all** for as long as the hand-rolled version existed. The consumer passes items and a model and supplies the body through the default slot; it never spells a relationship.
>
> ⚠️ **`aria-controls` IS ON THE SELECTED TAB ONLY, AND THAT IS THE SHARP EDGE.** Only the selected panel is in the document — which is what "the panels are already in the document" means for a set rendered one at a time — so putting the attribute on every tab would point every *unselected* one at an id that does not exist. That is worse than omitting it: a screen reader announces nothing and the reader cannot tell. §3.4.1 states the identical rule for `aria-activedescendant` and states the identical reason. ⚠️ **axe downgrades a dangling `aria-controls` to *incomplete* rather than a violation, so the merge gate stays green over the lazy version.**
>
> **ACTIVATION IS AUTOMATIC** — an arrow moves focus and selects. The APG sanctions that where revealing a panel is cheap, and these panels are already-mounted local state. Manual activation earns its keep only when selecting costs a round trip, at which point the thing being switched is probably a page and the component is probably `MdsTabNav`.
>
> **WHEN THE MODEL NAMES NO ITEM, NOTHING IS SELECTED BUT THE FIRST TAB STILL HOLDS THE TAB STOP — THE DELIBERATE OPPOSITE OF `MdsTabNav`, WHICH RESOLVES THE SAME CASE TO `-1`.** Both are correct for their own shape and must not be "aligned": `TabNav`'s items are links and stay focusable whatever it marks, whereas a roving tabindex with no `0` in it removes the entire control from the keyboard's reach. So the component never invents a selection the consumer did not ask for, and never becomes unreachable either.
>
> **THE PANEL TAKES NO `tabindex`.** The APG puts a panel in the tab sequence only when it holds nothing focusable; this one is designed to hold form controls. The implementation this replaced had `tabindex="0"` on a panel full of inputs **and then set `outline: none` on it**, so it minted a redundant stop in the app's tightest pane and made that stop invisible to whoever landed on it.
>
> **THE STRIP SCROLLS AND IS NOT FOCUSABLE**, for `MdsTabNav`'s reason verbatim: `scrollable-region-focusable` fires only on a scroll region with **no focusable descendants**, and every child here is a button.
>
> ⚠️⚠️ **THE RULE UNDER THE STRIP AND THE SCROLLING ARE ON TWO DIFFERENT ELEMENTS, AND THAT IS A CORRECTNESS DECISION THIS INCREMENT LEARNED BY MEASURING ITS OWN FIRST DRAFT.** Putting both on one element — plus the `-1px` bottom margin that pulls a tab's 2px underline over the container's own 1px rule — made the strip a **1px VERTICAL scroll container**: `overflow-x: auto` with the other axis unset **coerces `overflow-y` to `auto`** (CSS Overflow 3, the same rule §3.4a cites for the sidebar rail), and a negative bottom margin on a flex item in that box shrinks the container while the item's border box does not. Measured in the running builder at **`scrollHeight` 35 against `clientHeight` 34**, at 1440 and at 375. **Fifth instance of the containing-block/overflow class** after `MdsDataTable`, `MdsSegmentedControl`, `MdsSpinner` and `MdsTimeSeriesChart`.
>
> ⚠️ **NO GATE IN THIS REPOSITORY CAN SEE IT, INCLUDING THE ONE THAT LOOKS LIKE IT SHOULD.** happy-dom lays nothing out; the end-to-end assertion reads the **document's** scroll box, which `.app-shell { overflow-x: clip }` pins flat; and axe's `scrollable-region-focusable` fires only where the region has **no focusable descendants** — every child here is a button, so axe is *correctly* silent about a region that should not have been scrollable at all. **`MdsTabNav` never had this**, because its rule sits on the outer landmark and its scrolling on a separate child. **The generalisable rule: a horizontal scroll strip must not also carry the rule its items overlap — give the rule its own non-scrolling parent, and keep negative margins out of a scroll container entirely.**
>
> ⚠️ **PASS THE LABEL AS `ariaLabel`, NEVER IN KEBAB-CASE.** The kebab spelling is a real HTML attribute, so Vue resolves it as a fall-through attr: the name lands on the outer element and the tablist is left unnamed, while the page still *looks* labelled and axe — which sees a named element, just the wrong one — raises nothing. `vue-tsc` is the gate, and it caught it on this component's first call site.
>
> **ITS ONE CONSUMER IS `ConfigPanel.vue`, THE PRODUCT'S ONLY IN-PAGE TABLIST, AND THE EXTRACTION BOUGHT SCRUTINY RATHER THAN REUSE** — the same justification §3.4b records for `MdsMenu`. An application-tree component gets no story and therefore no `checkA11y` scan, and the builder's end-to-end specs click every tab on that page without ever asking what any of them points at. Three defects had survived that gap: the fill-token underline, the tabbable panel, and its suppressed focus ring.

> ⛔ **THE BUILDER PROHIBITION SURVIVED THE COMPONENT'S ARRIVAL RATHER THAN EXPIRING WITH IT (restated J4c, which built `MdsTabs` and adopted it into `ConfigPanel` on that very page without touching either control).** The note below was written when this component did not exist and reads as if it were waiting for one. It was not: **the builder's compact pane switcher and its Structure ⇄ Logic toggle stay `radiogroup`s permanently.** `ConfigPanel`'s tablist is the page's one and only, and that is now asserted in `ConfigPanel.test.ts` as well as argued here.

> ⚠️ **JR5 DELIBERATELY DID NOT BUILD THIS COMPONENT IN ORDER TO GET A PANE SWITCHER, AND WHEN J4 DOES BUILD IT, IT MUST NOT BE RETROFITTED ONTO THE BUILDER. Re-read this note first.** The reason is measured rather than stylistic. `builder-axe.spec.ts` runs **four loops that walk every `[role="tab"]` on the builder and click each one**, plus five `[role="tab"]…first()` settle locators and two `hasText: 'Advanced'` clicks; `responsive-axe.spec.ts` and `personalization-axe.spec.ts` add four more. `ConfigPanel` already owns a real tablist on that page, so a second one joins it in all thirteen: the loops would click pane-switch tabs in the middle of a scan — destroying the pane the scan was about to measure — and every `.first()` settle would resolve to whichever strip came first in the DOM. It would also be **the wrong ARIA**: the panes are the switcher's *siblings*, not its `tabpanel` children, so there is no `aria-controls` relationship to express and nothing for a tablist's contract to be about. A `radiogroup` claims none of that, and arrow-key roving comes free from native radios.
...
> **AS-BUILT (J4b). The contract above was written before the component and is met as written; what follows is only what implementing it decided.**
>
> **IT TELEPORTS TO `<body>` AND THE TELEPORTED ROOT CARRIES `data-mds-inert-exempt`.** Both are correctness rather than convenience. The teleport is what escapes the rail's clipping; the exemption is what keeps a tooltip usable on a control *inside* an open dialog, where the alternative fails three ways at once — dropped from the accessibility tree so `aria-describedby` resolves to nothing, given `pointer-events: none` so the *hoverable* clause becomes structurally impossible, and made invisible to the one person who is inside the dialog. ⚠️ **The attribute must sit on the direct body child with nothing between it and the `Teleport`** — the inert stack walks ancestor siblings, so an exempt node nested inside a node already marked inherits `inert` and the attribute does nothing, silently. In particular **do not wrap the bubble in a `Transition`**: besides re-nesting the attribute, the test utils stub a Transition as a real element, so the tree under test would differ from the browser's by a level. §3.4.1's exemption sentence is narrowed accordingly in the same increment.
>
> **`aria-describedby` REACHES THE TRIGGER THROUGH A SCOPED SLOT, AND THERE IS NO OTHER CONSTRUCTION THAT WORKS.** The anchor wrapper is not focusable, so an attribute on it is never announced; Vue cannot add an attribute to slot content; and reaching into the child from JavaScript breaks the moment a call site wraps its trigger. The call site binds `v-bind="trigger"` onto the real control, which also puts the contract in front of whoever writes it. The attribute is present **only while the bubble is**, never dangling — and note that axe downgrades a dangling `aria-describedby` to *incomplete* rather than a violation, so the merge gate would stay green over the lazy version.
>
> **ESCAPE IS A DOCUMENT LISTENER IN THE CAPTURE PHASE, AND THE PHASE IS THE FEATURE.** `MdsModal` binds Escape on its own panel, so a bubble-phase listener here would run *after* it and an Escape aimed at the tooltip would close the dialog underneath. Capture descends from the document, so the tooltip consumes the first Escape and the dialog receives the second — which is the layering 1.4.13 asks for. Focus does not move. ⚠️ **`MdsMenu` solves the same problem the opposite way, binding on its own root, because it always holds focus while open and a tooltip never does. Do not "align" the two: picking the wrong mechanism is invisible until something wraps a dialog around the component.**
>
> ⚠️⚠️ **AMENDED (J6): THE PHASE IS THE FEATURE, AND IT WAS ALSO A PAGE-GLOBAL ESCAPE SINK. THE FIX IS A PRIORITY RULE, AND IT IS THE FOURTH FACT THIS FAMILY NEEDED.** A capture listener on `document` runs before **every** Escape claimant on the page, whichever mechanism they chose, including ones this component has no relationship with. The paragraph above reasons only about a dialog *around* the tooltip, and is right about that case; it is written as though the tooltip were always in the reader's interaction context. **Measured in the running app at 800px, against BOTH shell mechanisms:** rest the pointer on a collapsed sidebar rail item with a shell popover open and press Escape — the bubble vanished and the **popover stayed open**. `useDismissable` binds Escape on `document` in the *bubble* phase, so it never ran at all; `MdsMenu`'s root handler and `MdsModal`'s panel handler lose the key the same way. ⚠️ **And one factual correction worth carrying, because the obvious guess is wrong: the ACCOUNT MENU is not a `useDismissable` consumer** — it moved to `MdsMenu`, and that file's docblock says so while still *naming* the composable, so a grep for the name matches the comment explaining its absence. The composable's real consumers are the **notification bell** and the **feedback button**. Both mechanisms were driven, because a fix verified against only one of them would have proved half of it.
>
> **THE RULE: DISMISSAL IS UNCONDITIONAL, CONSUMPTION IS SCOPED.** The bubble always hides — 1.4.13 requires it be dismissible, **not** that it be dismissible *exclusively* — but `stopPropagation()` runs only when the tooltip is in the context the key was aimed at, which is **wherever focus is**: the anchor contains `document.activeElement`, or nothing holds focus at all (so nothing else can be claiming the key). Focus elsewhere means hide *and let the key through*.
>
> **THE TRADE IS DELIBERATE AND BELONGS IN THIS DOCUMENT RATHER THAN A COMMIT MESSAGE:** a **hovered** tooltip inside a dialog, with focus on some other control in that dialog, now dismisses **and** lets the dialog close, where before it ate the press. That is the right way round — a pointer user resting on a description while pressing Escape means *close the dialog*, and eating the key there loses the whole task to hide a label. The layering the paragraph above describes is preserved exactly where it was argued for: the control **focused**.
>
> ⚠️ **THIS DOES NOT RETIRE THE RAIL-BAND GATE BELOW, WHICH READS AS THOUGH IT MIGHT.** Under 480px the drawer moves focus onto a rail item *programmatically*, so the anchor **would** hold focus and the tooltip **would** still take the drawer's first Escape. The gate is still the fix for that, unchanged.
>
> **HOVERABLE IS A TRANSPARENT `::before` BRIDGE PLUS A 120ms GRACE ON DISENGAGEMENT.** The bridge covers the 8px gap so a pointer travelling from trigger to bubble never leaves both at once; its width and the geometry module's offset are one number spelled twice, and a test holds them equal — if they drift, the tooltip dismisses itself mid-journey and it reads as a flaky hover rather than as two constants that stopped matching. The grace is on *disengagement* only and is not the auto-hide the contract forbids.
>
> **A TOOLTIP AND A NATIVE `title` ARE MUTUALLY EXCLUSIVE ON ONE ELEMENT** — two tooltips render one under the other. The sidebar's two `title` bindings were deleted in the same increment. ⚠️ `MdsIconButton` still sets `:title` from its `label`, so **the first consumer that layers a tooltip on an icon button owes it a prop to suppress that, in the same PR as the consumer** — not before, which would be an unconsumed API on a component this very section warns about.
>
> **THE GEOMETRY IS A PURE MODULE ON PLAIN RECTS, AND THAT IS FORCED BY THE TEST ENVIRONMENT RATHER THAN CHOSEN.** happy-dom computes no layout, so every `getBoundingClientRect()` is zeros and a DOM-level flip assertion would pass against an implementation that never flips. ⚠️ **It also produced the increment's sharpest defect: a zero-area rect read as "off screen", so the tooltip showed, measured itself on the next tick, concluded its own trigger was gone, and hid in the same frame — it never appeared for any user action. Every case that seeds visibility directly PASSED, and seeding visibility is exactly what the axe stories do**, so the accessibility job would have been green over a tooltip nobody could open. A zero-area rect means *unmeasured*, not *absent*.
>
> **WHERE IT IS ON, AND WHY THAT IS NARROWER THAN §3.4's LINE SUGGESTS.** The sidebar enables it only across **481–1024px**. Above that the label is in the item. Below it the drawer restores the label and there is no hover — but focus still fires, and the drawer moves focus programmatically on open, so an ungated tooltip would appear unbidden and then eat the drawer's first Escape.
>
> **ON TOUCH, STATED PRECISELY BECAUSE THE HALF-ANSWER IS TEMPTING.** A `pointerenter` whose `pointerType` is `touch` is refused outright — on touch that event is the first half of a tap, and a bubble torn down by the navigation it preceded is worse than none. The rail's labels are in the accessibility tree regardless (the clip idiom hides them from sight, not from AT), so **a screen-reader user on any device gets the destination name from the link itself and never from the tooltip** — that is what makes the tooltip an enhancement rather than a carrier. The case that remains uncovered is a *sighted touch user between 481 and 1024px*: no hover, and the visible label clipped. It is accepted rather than solved, on three grounds — the drawer restores visible labels below 481px, the rail is a fixed set of at most ten destinations with deliberately distinct glyphs, and every destination names itself in its own `h1` or breadcrumb, so a wrong tap is self-correcting and one tap from recovery. **Do not claim the sr-only span alone closes it.**

### 3.4b Menus — `MdsMenu` *(**AS-BUILT since J4b**)*

The anchored action menu: a trigger that discloses a short list of destinations and commands. Extracted from the account menu, which had been the only implementation of this pattern in the product.

**⚠️ THE EXTRACTION BOUGHT SCRUTINY RATHER THAN REUSE, AND THAT IS THE JUSTIFICATION FOR MOVING IT WITH ONE ADOPTED CONSUMER.** §1.3's consolidation trigger is three-plus deviations for the same need, and this had them — two hand-rolled anchored panels plus a third that `FormRowActions.vue` explicitly *refused to write* for want of this component. But the decisive argument is different: **an application-tree component gets no Storybook story and therefore no `checkA11y` scan**, and no end-to-end spec ever opened the account menu. Moving the markup into the package is what finally ran `aria-required-children` over it, and it immediately found a real defect.

**A MENU OWNS ONLY MENUITEMS.** The version this replaced put a name/email `<div>` **inside** `role="menu"`, whose children must all be `menuitem`/`menuitemradio`/`menuitemcheckbox`/`group`. That is not a misplacement to tidy: a screen reader in application mode walks the menuitems and never announces a stray `<div>`, so *"who am I signed in as"* was unreachable to precisely the readers most likely to want it. The panel is therefore a plain box, the header is its first child, and `role="menu"` is a **nested** element containing nothing else — with the header wired as the menu's `aria-describedby`, so entering the menu announces it. Nothing moved visually.

**KEYBOARD**, per the APG menu pattern: `ArrowDown`/`ArrowUp` move and wrap over the items; `Home`/`End` jump to the ends; `Enter`/`Space` activate; **`Tab` closes without `preventDefault`**, so the tab sequence continues into the page behind it; `Escape` closes and returns focus to the trigger. Items are `tabindex="-1"` — a roving set, not five tab stops.

**⚠️ ESCAPE BINDS ON THE COMPONENT'S OWN ROOT, NOT ON `document`, AND THE DIFFERENCE IS INVISIBLE UNTIL SOMETHING WRAPS A DIALOG AROUND IT.** `MdsModal` listens on its panel, an ancestor of any menu opened inside a dialog, so a document-level bubble listener — which is what the account menu and the application's dismissal composable both used — is the *last* thing to see the key: the dialog closes first, and the `stopPropagation` written to prevent exactly that never runs. The user loses the whole task instead of a two-item popup. Binding on the root puts this handler first, because the event starts at the focused menu item inside it. This is safe only because a menu always holds focus while open; **`MdsTooltip` inverts the mechanism for the opposite reason** (§3.4a), and the two must not be "aligned".

**`aria-disabled`, NEVER THE NATIVE ATTRIBUTE**, for §3.4a's reason: a natively disabled control leaves the arrow-key ring and the accessibility tree, taking any explanation of *why* it is unavailable with it. Because `aria-disabled` is advisory, the click still arrives and the handler must refuse it — an implementation that only sets the attribute looks completely correct and still fires.

**HETEROGENEOUS ITEMS.** An item with an `href` renders through `linkComponent`; one without renders a real `<button>` and emits `select`. The `linkComponent` contract is `MdsBreadcrumb`'s verbatim, including the part that matters: **the injected component must receive `href` as a real prop**, since bound as a fall-through attribute the anchor looks right and silently stops being a client visit. The default stays the string `'a'` so Storybook renders with no router present.

**IT DOES NOT TELEPORT, AND THE MEASUREMENT IS WORTH RECORDING BECAUSE IT DIFFERS FROM §3.4a's.** Nothing in the panel's ancestor chain sets `overflow` until the app shell's `overflow-x: clip` — and `clip` paired with `visible` does **not** drag the other axis to `auto`; that coercion is specified for `hidden`, `scroll` and `auto`. So there is no scroll container and no vertical clipping, unlike `.sidebar`. ⚠️ **But `overflow-x: clip` has a sting in the other direction: an over-wide panel is CLIPPED rather than caught**, because the document-level overflow assertion the e2e suite relies on reads a width the clip pins flat. Content can widen this panel — its header carries a user's email, and §2.9's text-size axis scales that by up to a quarter. The panel is therefore clamped in viewport units with a wrapping header, and **that clamp is the only control there is**.

**⚠️ CONSEQUENTLY `MdsMenu` MUST NOT BE MOUNTED INSIDE A SCROLL CONTAINER** — not `.mds-modal__body`, not the data table's scroll region, not the collapsed rail. Those need §3.4a's teleport-plus-exemption construction; a consumer that needs one adds it there rather than making every menu in the product float.

**THE ANCHORED-PANEL RULE, generalised from the two measurements this product has.** A panel around **220px** wide, right-aligned in the top nav, starts near +139px at 375px and is comfortably contained — it stays anchored. A panel at **360px** would start near −69px and be clipped, which is why `NotificationBell` carries a ≤480px sheet fallback and this does not. Pick by measuring, not by copying whichever precedent you saw first.

**⚠️ `FormRowActions.vue` IS DELIBERATELY NOT ADOPTED, AND THIS IS RECORDED SO IT IS NOT READ AS AN OVERSIGHT.** Its docblock names the missing primitive as one of two blockers, and J4b clears only that one. **Five of its nine buttons are pinned by end-to-end specs** — `templates-axe.spec.ts` clicks *Save as template* unscoped, so it must be in the accessibility tree with no menu open, and `responsive-axe.spec.ts` navigates by *Response statistics* and *New submission* from four call sites. Folding them behind a trigger is a spec change as much as a UI change, and `tests/e2e/support/navigate.ts` records that one change to how that row navigates once cost **81 failures across four spec files**. It is a separate increment with its own spec churn, and an unadopted consumer is not a deviation.
...
> ⚠️ **AND THAT ONE-PRIMARY RULE IS WHY THE FIRST-RUN MOMENT IS A CARD GRID RATHER THAN AN EMPTY STATE (J5c).** `docs/onboarding-template-content-plan.md` §2 asks for **two equally-weighted choices** — *start from a template* and *start from blank*. Two equal primaries cannot be expressed in this component without breaking the rule above, and weighting one of them (as the forms list correctly does, one primary + one tertiary) is not what §2 is asking for. §2 names the right pattern in its own sentence — *"presented as the standard **card-grid** pattern"* (§3.5) — so the two documents never disagreed; the resolution is recorded here because reaching for `MdsEmptyState` is the obvious move and it is the wrong one. **`resources/js/Pages/forms/Index.vue`'s empty state is unchanged and remains correct**: it is a list with nothing in it, which is what this section is for.
>
> The permission-restricted arm above is what the dashboard falls back to when the reader cannot author at all — explanation, no CTA — and it is the third of J5c's three first-run states, after *both cards* and *blank card alone* (the latter being ADR-0011 §D9's absent-not-locked posture for an unentitled template gallery).
...
### 3.13 Checklists *(**AS-BUILT since J5b** — `MdsChecklist`)*

A short, finite list of things a workspace has or has not done yet, each row either ticked or offering the one link that would tick it. Its only consumer today is the dashboard's getting-started card (`docs/onboarding-template-content-plan.md` §2); the shape generalises to any "set-up progress" surface.

**Structure**: a title, an `MdsProgress` meter, and a `role="list"` of rows. Each row is a **mark** (the `check` glyph when done, a CSS ring when not), a **label** (a link when the row is actionable, plain text otherwise), and an optional one-line description.

⚠️ **THE VISUAL GRAMMAR IS `MdsPasswordStrength`'s, DELIBERATELY AND EXACTLY** — the same `check` glyph, the same 16px CSS ring, the same visually-hidden state word. Both components are *a list of conditions, each met or not, with a running count*, and a reader who has seen one must recognise the other. The one difference is what earns a second component rather than a prop on the first: **a checklist row can be actionable.** A password rule is something the user is already doing; a checklist row is somewhere they have not been yet.

**The state is never carried by colour or by the glyph alone** (WCAG 1.4.1): the mark is `aria-hidden` and every row opens with a clipped `Done:` / `To do:`. The done row's colour is `--mds-color-status-success-fg` — `-fg`, never the `-bg` half, per §3.4's standing rule for any coloured indicator, which this project has now paid for three times (J2a's tab strip, J4a's accent bar, J4c1's underline at **2.41:1** in teal dark).

⚠️ **THE METER IS `MdsProgress` AND IS NOT §3.9's STEP-COUNT VARIANT, WHICH REMAINS UNBUILT AND SHOULD.** The step-count variant is *"Step X of N"* for multi-step **form navigation**, carrying a rule a checklist has no use for — a step is done because the respondent *visited* it, never because its index is lower. A checklist has no current position, no ordering to be ahead or behind of, and no navigation. Reaching for it here would have generalised a component against a shape it does not have. What a checklist wants is the percentage variant with a domain-native `valueText` — `2 of 4`, because `aria-valuenow="2"` against `aria-valuemax="4"` is otherwise announced as *"50 percent"*, which is not a thing anybody counts a to-do list in. **The meter's label must differ from the card title**, since `MdsProgress` renders its own label unconditionally and passing the title twice prints it twice.

**Two degradations, and the component cannot tell them apart on purpose.** A row with no `href` renders as text, whether because it is **done** (nothing left to do) or because the **reader is refused its destination**. Only the caller knows which. What the component must never do is *drop* the row: dropping it would make one workspace show a different **number** of steps per role, which is the hierarchy loss `MdsBreadcrumb` refuses for the same reason. ⚠️ Deciding *which* rows exist and where they point is the caller's, and it belongs on the **server** — `app/Services/Onboarding/GettingStartedChecklist.php` mirrors each destination's real gates through `ShellAbilities` and `FeatureAdmission`, on the `CrumbTrail`/`FormTabSet` precedent that a href resolved in a client `computed` sits where no Pest test can reach.

**It never hides itself.** `dismissible` renders the control and `dismiss` is emitted; the `v-if` stays at the call site, exactly as `MdsAlert` and `MdsToast` draw the line. Whether a dismissal survives a reload is a page decision every time — the dashboard's answer is a column on the membership row, and an optimistic local hide would make the card vanish even when that write failed.

**`linkComponent` defaults to `'a'`** so the package imports nothing from the application's router; an Inertia page passes its own `Link`. The `MdsBreadcrumb` contract, for the `MdsBreadcrumb` reason.
...
- **Modal / dialog focus-trap**: `role="dialog"` (or `role="alertdialog"` for the destructive-confirmation variant, §3.6), `aria-modal="true"`, `aria-labelledby` pointing at the modal's visible title. On open: focus moves to the modal's first focusable element (or a designated initial-focus target — e.g., the cancel button by default for destructive confirmations, so an accidental `Enter` press doesn't confirm a destructive action). While open: `Tab`/`Shift+Tab` cycle only within the modal's focusable elements (a true focus trap, not merely a visual overlay); `Escape` closes it (equivalent to the cancel action). On close: focus returns to the element that triggered the modal, never left stranded at `document.body`.
  ...
  - **As-built amendment (Increment J6): the RETURN half is now verified too, and it was not — which made "never left stranded" a promise this document made and the component did not keep.** `initialFocus`'s amendment above says a target that cannot take focus *must fall back, and the component verifies this after focusing rather than trusting the lookup*. The same sentence was owed on the way out and was missing: `closePage()` called `.focus()` on the captured opener and trusted it. **An opener can die between open and close in three ways, all silent** — it is removed from the document (an Inertia visit re-rendering the shell), it stops being rendered (a control that only exists above a breakpoint), or it stays inside an `inert` subtree because a non-dialog **surface** is still holding the page underneath. `MdsModal` now gains a `returnFocus` prop (selectors in **preference** order, resolved against the document) and walks opener → `returnFocus` → whatever surface still owns the page, verifying after each.
    - ⚠️ **THE VERIFICATION ASKS "IS FOCUS STRANDED?", NOT "DID THE OPENER TAKE IT", AND THAT DISTINCTION PRESERVES THE PARAGRAPH BELOW FOR FREE.** §4.5's own I10a note records that focus is deliberately *not* returned when a lower modal closes under an open upper one. A check written as *did the opener get focus* would have broken exactly that; a check for stranding leaves it untouched, because focus is legitimately inside the upper panel.
    - ⚠️ **AND IT RUNS ON THE NEXT TICK, WHICH WAS MEASURED RATHER THAN REASONED.** Verifying synchronously in the close watcher fails: the watcher is pre-flush, so the panel is still mounted and focus is still legitimately inside it. **The strand happens one tick later**, when the `v-if` tears the panel out and the user agent drops focus to the body. Checking early does not measure the failure, it measures the moment before it — the mirror image of why the take path is already in a `nextTick`.
    - ⚠️ **`checkVisibility()` IS BLIND TO `inert`, AND THIS IS THE GENERAL RULE THAT FALLS OUT.** It answers about **rendering** — `display`, `visibility`, `content-visibility` — and an inert element renders perfectly normally, so "is this visible?" returns `true` for a control `.focus()` cannot move to. Any code choosing a focus target has to ask both questions, and it must ask about `inert` with `closest('[inert]')` rather than about the element alone, because `inert` is **inherited** and the attribute sits on an ancestor in every real occurrence. One predicate, `firstFocusable`, now answers both for the whole system (`packages/design-system/src/components/Modal/focus-target.ts`); the command palette had a private half-answer and that is where the defect lived.
...
#### 4.6.1 ⚠️ THE COVERAGE GAP: STORYBOOK GLOBS THE PACKAGE ONLY, SO AN APP-TREE COMPONENT IS NEVER SCANNED *(promoted here in J4c; cite this section, not an exceptions-log entry)*

**Storybook globs `packages/design-system/src/**/*.stories.@(ts|tsx)` and nothing else.** A component that lives in `resources/js/` therefore gets **no story and no `checkA11y` scan at all** — so a green `design-system-a11y` job says *nothing whatsoever* about it. This is a fact about the glob rather than about any one component, and it is stated here because **four separate files were citing an exceptions-log entry for it**, an entry that was always about the command palette specifically and that J4c retired. **Cite `§4.6.1`.**

**IT IS THE REASON SEVERAL COMPONENTS LIVE IN THE PACKAGE RATHER THAN THE APP TREE**, and the reason is coverage, not taxonomy: `MdsFilterBar` (a heading-order contract that only fails in the empty state), `MdsTabNav` and `MdsBreadcrumb` (`aria-current` exactly once; list semantics surviving `list-style: none`), `MdsPasswordStrength` (a component whose whole job is announcing state changes to somebody who cannot see them).

⚠️ **AND MOVING MARKUP HERE HAS FOUND A REAL DEFECT EVERY TIME, WHICH IS WHAT MAKES THIS AN ARGUMENT RATHER THAN A PREFERENCE.** J4b's `MdsMenu` extraction immediately failed `aria-required-children` on a `role="menu"` that had held non-menuitem children since it was written. J4c1's `MdsTabs` extraction surfaced a tablist with no `aria-controls`, a tabpanel in the tab sequence, and a suppressed focus ring on it. Each had shipped for months behind a green accessibility job.

⚠️ **WHAT THE SCAN STILL CANNOT SEE, EVEN IN THE PACKAGE.** axe does **not resolve** `aria-activedescendant` or `aria-controls`, so a relationship naming an id that is not in the document passes every scan while announcing nothing — it is reported as *incomplete*, never as a violation. Those must be asserted in the unit suite, which is why `MdsCombobox` and `MdsTabs` both carry cases that look redundant beside a green axe job and are not.
...
- **The sidebar nav** (§3.4) has three responsive states, all component-owned: full expanded (desktop, > 1024px), icon-only collapsed with hover/focus tooltips (tablet, ≤1024px), and a bottom-anchored icon tab bar or hamburger-triggered full-screen overlay drawer (mobile, ≤480px) — a page never needs to know which of the three is currently rendering.
  ...
  > **⚠️ AS-BUILT (J4b): THE MOBILE STATE IS A 260px LEFT FLYOUT WITH A SCRIM, NOT A FULL-SCREEN OVERLAY, AND THE SPEC IS AMENDED TO THE ARTEFACT RATHER THAN THE OTHER WAY ROUND.** A full-screen overlay leaves no visible page, which removes the scrim — and the scrim is both the drawer's primary dismiss affordance and the only visual cue that it is transient. The flyout keeps both.
  >
  > **AND THE LAST CLAUSE OF THIS BULLET IS NOW BINDING RATHER THAN DESCRIPTIVE: THE DRAWER IS THE SAME `<nav aria-label="Primary">` ELEMENT AT ALL THREE WIDTHS AND MUST NOT ACQUIRE `role="dialog"` AT ONE OF THEM.** J4b took that obvious-looking fix off the table for exactly this reason — making the primary navigation landmark's identity depend on the viewport is what this bullet forbids, and at 375px the landmark would disappear from a screen reader's landmark list. What the drawer does instead is **take the page**: `useInertBackground` marks everything outside it `inert`, which is a stronger guarantee than a Tab trap because it also covers the virtual cursor and the browser-chrome round trips a `keydown` handler never sees. Two consequences, stated here rather than left to be discovered. **The hamburger is itself inert while the drawer is open** — the inert exemption cannot help, because it only works on an element the ancestor-sibling walk actually visits and the hamburger is nested inside the header that gets marked. ⚠️ **SO THE DRAWER CARRIES ITS OWN LABELLED CLOSE BUTTON, AND AN EARLIER DRAFT OF THIS PARAGRAPH GOT THAT WRONG.** It said Escape and the scrim were sufficient "exactly as `MdsModal` treats its opener" — a false comparison, caught by J4b's adversarial pass: **`MdsModal` ships a labelled close control *inside* its panel for precisely this reason.** The scrim is a bare `div` with no role, no accessible name and no tab stop, so screen-reader swipe navigation never lands on it and switch access cannot reach it at all. Without a close button in the drawer, a VoiceOver or Switch Control user who opened it could only leave by navigating somewhere else. **Any surface that takes the page owes a dismiss control that survives its own inerting.** And **⌘K is refused while the drawer is open**, which is correct and also removes a hazard, since both palette openers sit in the now-inert top nav where `.focus()` is a silent no-op.
