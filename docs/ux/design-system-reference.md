...
> **⚠️ SIX HUES REQUIRED DOUBLING UP, AND THE ARITHMETIC IS THE INTERESTING PART.** The semantic palette already occupies FOUR of the six perceptual colour families — red (Danger), amber (Warning), teal-green (Success) and blue (Primary/Info) — and the chart scale spends five more. What is genuinely unclaimed is yellow-green, green, indigo, magenta and rose: **five families for six slots**. So the scale doubles up symmetrically, three in the green band and three in the purple band, ~34° apart within each, rather than crowding one side and leaving the other thin. The index ORDER alternates the two families, so two consecutive indices are never same-family.

> **⚠️ THE BINDING CONSTRAINT IS THE STATUS HUES, NOT THE CHART SERIES.** The status pill is co-located on the card; the chart series never appear on this page at all. Minimum measured separation from any semantic status hue is **33°**; minimum mutual separation within the scale is **34°**. Both are asserted — the status hues are resolved through `semantic.json` rather than copied, so re-hueing Warning in some later row surfaces the collision in the test instead of on the page.
...
Two bars apply, and they are different: the glyph initials are small bold TEXT ⇒ 4.5:1 (all six measure 4.52–4.67 on their own tint, in both themes); the 4px card edge is a non-text indicator ⇒ 3:1 (WCAG 1.4.11), measured against **both** grounds a card can sit on — ≥4.99 on the light canvas and ≥6.17 on the dark one. Per the J2a finding, an edge is an `-fg`-strength value and never a `-bg` fill.
...
>
> **Registered, not fixed: the retype makes headers wider, and the gate that would catch it runs where headers are hidden.** Uppercase glyphs are ~15–25% wider than lowercase and `--mds-tracking-wide` adds ~6%, against an ~8% size reduction — net roughly +15–20% on a `white-space: nowrap` header, which is what sets a table's minimum content width. On the widest tables that can push `.mds-table__scroll` into horizontal scrolling at viewports that previously fit. It does **not** overflow the page (the wrapper scrolls, that is its job), and the `responsive-axe` overflow sweep asserts at 375px — where `thead` is visually hidden, so the gate literally cannot render the thing that got wider. This is the 481–1024px tablet band that **JR4** exists to address; it is named here so JR4 inherits a known cause rather than a symptom.
>
> Related and intended: `.mds-table__td::before`, the mobile key/value label, inherits from `__td` and not from `__th`, so the same column header reads uppercase-tracked on desktop and sentence-case on the card-per-row. That is correct — on mobile it is a field label beside a value, not a column heading — but it is a real divergence and was not obvious.
>
