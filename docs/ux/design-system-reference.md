...
- **In every axis, the product default is the ABSENCE of the attribute** — `system` mode, `blueprint` accent, `standard` size, dyslexia off. There is no `data-font-size="standard"` rule and there never should be. This is what makes an un-personalized user render byte-identically to a guest, and it is why the blade emits each attribute through a whitelist (`in_array`) rather than interpolating the prop, so a corrupted row cannot inject an arbitrary attribute value.
  - **One narrow exception, on the accent axis only, once a tenant brand ramp exists ([ADR-0014](../adr/0014-tenant-brand-ramp-generation.md), Increment H23a3).** The tenant ramp occupies plain `:root`, so on a branded tenant the *absence* of `data-accent` stops meaning "product default" and starts meaning "inherit this organisation's brand" — leaving a member who wants the product blue back with no way to express it. Where, and **only** where, the tenant has an active ramp, the shell therefore emits an explicit `data-accent="blueprint"` and a `:root[data-accent='blueprint']` rule restores the product ramp. On an unbranded tenant nothing changes and absence-as-default holds exactly as written above. The whitelist requirement is untouched — `blueprint` joins it as a permitted value rather than being interpolated.
