...
## #9 — Hand-rolled command-palette combobox (`resources/js/components/search/`)

**What deviates:** DSR §4.5 specifies the ARIA 1.2 Combobox pattern and §3.2's input table points at it for
"searchable, cascading" variants — but `packages/design-system/src/components/` contains no Combobox, no
Listbox and no Menu. The ⌘K command palette therefore implements the pattern **in the app tree**, not as a
shared primitive. It is the second surface to do so: `AccountMenu.vue` already hand-rolls `role="menu"` with
arrow-key roving for the same reason.

**Why:** The increment that owns the ~15 missing primitives runs *after* this one, and pulling `MdsCombobox`
forward means specifying it for cascading selects, async loading, multi-select and free-text — none of which
the palette needs — under the merge-blocking Storybook axe job, inside an increment that already touches the
nav, a new page, four list pages and the database. There is also a design argument against generalising from
this instance: a palette's options are *navigation targets*, it writes no value back to a model, and it has
no selection that persists after activation. A primitive extracted from it would be the wrong primitive.

The narrower precedent is the DSR's own Tabs note (§3.4), which says in as many words that the increment
first needing a missing primitive "either builds it to this spec or uses the existing `SegmentedControl`; it
cannot import one." This is that situation with a component that has no stand-in at all.

**Disposition:** Accepted as **temporary and named**. Three things bound it. (1) The behaviour is specified in
DSR §3.4.1 *before* it was built, so the eventual primitive has a written contract to satisfy rather than an
implementation to reverse-engineer. (2) **When `MdsCombobox` is built, the command palette is its first
refactor target** — that is the consolidation trigger, recorded in both places. (3) No second palette may be
built in the meantime; a page needing this shape waits for the primitive or extends this one.

⚠️ **It carries a coverage gap that the design-system a11y job cannot close, and this is the part worth
remembering.** Storybook globs `packages/design-system/src/**/*.stories.@(ts|tsx)` only, so an app-tree
component gets **no story and no `checkA11y` scan**. The palette's only automated accessibility gate is
Playwright. A green `design-system-a11y` job says nothing whatsoever about this component — do not read it
as coverage.

**Also recorded here, because it is the same root cause pointing the other way:** `MdsModal` gained an
`initialFocus` prop in the same increment rather than the palette working around the default. That is **not**
an exception — DSR §4.5 has specified a "designated initial-focus target" since the component was written and
the half had simply never been built. Fixing the primitive was available, so the deviation was not taken.
