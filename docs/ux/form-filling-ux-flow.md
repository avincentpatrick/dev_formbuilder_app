...
> **Amendment (Increment M107, `D36`) — A CORRECTION IS NOT RESUMABLE, AND THAT IS THE ANSWER RATHER THAN A GAP.** Everything above describes ENCODING — a keyer building a response that has not been submitted yet. **Correcting an ALREADY-SUBMITTED response is a different channel and autosaves nothing:** `EncodeFormPresenter` sends a null `draft_url` in edit mode and `Encode.vue` disables autosave there, both deliberately, and `SubmissionEditRoutesTest` pins `draft` and `draft_url` null so it cannot drift. ⚠️ **The reason is not that it was too hard.** The only write path a correction has is `PATCH /submissions/{submission}/answers`, and `SubmissionAnswerEditService` sends an Approved response back to **UnderReview** and writes an audit ledger row on **every call** — so a debounced autosave there would mean one review demotion and one ledger row *per keystroke batch*, which is a different product rather than a smaller version of this one. `D36` chose the button: **a correction is kept when Save is pressed, and not before.** The leave guard warns an editor before they navigate away, so work is lost only to a browser crash. A staff member who wants a resumable working copy does not have one, and should not be told otherwise.
>
...
> ⚠️ **AMENDED IN INCREMENT M21 (2026-08-26) — "SESSION/DEVICE-SCOPED" HAD ONLY EVER BEEN BUILT AS "DEVICE", AND THE SENTENCE ABOVE IS WHY THAT WAS EASY TO MISS.** The guest draft was keyed by form version + share-link slug with **no respondent dimension at all**, so an ABANDONED fill was restored into the *next* respondent's form — silently, with their answers on screen, no banner, and the header pill reading "Saving…" then "Saved" over a stranger's data. Rows now carry a `respondent_session_id` (`docs/adr/0021-respondent-scoped-device-outbox.md`, amended by M21) and a restore is refused unless the visit matches.
>
> ⛔ **THIS DOCUMENT DID NOT SETTLE THE QUESTION, AND IT SHOULD NOT BE CITED AS THOUGH IT DID.** The bullet above opens *"session/device-scoped"* — which reads as decisive — and then, in the **same sentence**, says *"a reload of the same browser tab **or the same device's browser** recovers the draft"*, which authorises the broader reading. Both are here, one clause apart. What settles it is that **every sentence in §5.1 is written about *the* respondent, definite article, singular**: this section assumes one person per device from end to end, and the shared-kiosk case is the one nobody wrote down. The narrow part that *is* load-bearing is the last sentence — the promise is bounded to the fill in progress, and "come back later" is §5.2's promise, delivered by a signed respondent-bound resume link.
>
> **The cost, stated rather than discovered later:** on a **personal** device, closing the tab mid-fill and returning now loses the draft, because the visit lives in `sessionStorage` and dies with the tab. On a form with save-and-resume the respondent has §5.2's emailed link; on a form without it (free tier, per the tier boundary below), the work is gone. The trade is one silent cross-respondent disclosure of **answer content** — which also reached the server draft and an emailed resume link — against one personal-device respondent losing an unfinished draft after closing their tab.
>
> ⚠️ **AND THE 30-SECOND BACKSTOP NAMED ABOVE WAS UNCONDITIONAL, WHICH QUIETLY VOIDED THE SEVEN-DAY EXPIRY IN THIS VERY BULLET.** It rewrote the draft's timestamp every thirty seconds whether or not anybody had typed, so an open tab's draft could never age out — the only thing that expires a draft is a timestamp that stops moving. It now skips the write when nothing changed, which is what makes the "7 days of **inactivity**" decision above true as built.
...
- ⚠️ **A RESUMED DRAFT HAS TWO WRITERS, AND THE RESPONDENT IS TOLD RATHER THAN SILENTLY OVERRULED (Increment P3a, 2026-08-17).** The resume link is what creates the case: it carries the **same** `client_submission_uuid` to the second device, so both devices are editing one server draft rather than two. The saves are whole-document replaces, so before P3a whichever device saved second silently reverted the other's answers, with nothing shown to either person. **Now the stale device is refused** — the server compares the state each save was *based on* against what it holds, and answers `409 draft_conflict` when they differ. ⚠️ **This never fires for ordinary filling**, on either channel: a device that keeps saving its own work always matches its own base. It fires only for the genuinely two-device case, and the copy names the remedy — reload the draft to pick up the newer answers, then carry on — because "conflict" alone leaves someone pressing Save into the same refusal. The encode page's autosave stops and says the same thing rather than retrying a stale write forever. **No automatic merge of the two copies is attempted; that is the CRDT work deferred in `docs/offline-first-sync-design.md` §9.**
...
8b. **A CORRECTION to an already-submitted response is saved on the button and is not resumable** (`D36`, M107) — the edit channel demotes review status and writes an audit row on every call, so autosaving it would be a different product (§5.1's M107 amendment).
