...
**Built as of I3**: `submission.created`, `submission.approved`, `submission.returned`, `form.published`, `form.opened`, `form.closed`, `member.invited`. **Still unbuilt** (named above as starter catalog, no emission site yet, and therefore NOT in `DomainEventType` — a case a tenant can subscribe to but which never fires is worse than a missing one): `submission.updated`, `form.archived`, `subscription.updated`.
