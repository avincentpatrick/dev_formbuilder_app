...
| 3 | Slack | A dedicated OAuth-based connector (Slack App with incoming-webhook or bot-token scopes) — richer than the generic webhook mechanism (channel picker, formatted message blocks), built as its own small service on top of the same underlying event catalog (§3), not a replacement for it. |
