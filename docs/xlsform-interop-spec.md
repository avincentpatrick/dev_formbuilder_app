...
| `hidden` | `calculate` (no visible widget) when the value is computed; a `text` field with `appearance: hidden` when it holds a literal default | Full for the computed case; the literal-default case depends on the target ODK/Kobo client actually honoring `appearance: hidden` (most modern ODK Collect versions do). ⚠️ **Increment H7 added a prefill SOURCE (`config.prefill_source` / `config.url_param`) that has no XLSForm expression at all** — XLSForm has no notion of "fill this from the sharing link", so an exported hidden field keeps its literal (via `default`) but silently loses whether that literal was authoritative and loses a `url` source entirely. A re-import therefore yields a source-less hidden field, which holds nothing. Recorded rather than worked around: inventing a `#meridian` marker for it would encode a web-link concept into a paper/mobile form spec. |
