...
export const NOTIFICATION_VISUAL: Record<NotificationTypeKey, NotificationVisual> = {
    ...
    // K1e — ⚠️ THE PLACEHOLDER IS DISCHARGED. K1b set this to `trend-up` and said in as many words that it
    // was a substitute, because the registry held no award, trophy, medal or star and adding one meant
    // editing `packages/design-system/` for a mark no other surface needed yet. K1e builds the surface that
    // needs it, so the `award` glyph now exists and this entry points at the honest mark. Note what the
    // swap gives BACK: `trend-up` returns to meaning a rising metric alone, which is what `MdsStatTile`
    // spends it on for every positive delta — the collision was real, not merely inelegant.
    badge_earned: { icon: 'award', variant: 'success' },
