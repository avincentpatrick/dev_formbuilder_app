 ...
 * LONG A FORM TAKES. `touch()` runs on every read, and the runtime reads this on every refresh of
 * the sync surface, so a respondent filling a long form never crosses it. The only exposed window
 * is AFTER finishing, where ten minutes is short enough that someone arriving at a shared desk
 * almost always crosses it and long enough to survive reading a confirmation screen. A shorter or
 * operator-configurable value belongs to the kiosk-mode row `docs/feature-backlog.md:68` already
 * parks ("lock to one form, auto-reset, clear PII on timeout"), not here.
 *
