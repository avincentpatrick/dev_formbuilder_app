...
function finish_row(array $row): array
{
    ...
    // Liveness is reported here and GATED FOR PRESENCE by tests/Feature/Docs/BacklogProvenanceTest.php
    // since M65, which backfilled the 30 open rows that carried no marker. M64's note here said the
    // marker was deliberately not gated because deciding it is a judgement against the code — the M37
    // triage job — rather than a text edit. That was true while the judgements were unmade; the
    // backfill is the making of them, and the gate now holds the NORMALISATION rather than the
    // judgement. Neither the gate nor this reporter checks that a verdict is CORRECT.
    //
 ...
 */
/**
 * The one ordered line, and how far behind the trunk it has fallen.
 *
 * ⚠️ THIS READS THE PROVENANCE LINE, IT DOES NOT INVOKE THE GENERATOR. `scripts/pipeline.php` shells
 * `backlog-triage.php --json`, which shells this file — so calling it here would be a cycle. The
 * staleness question is answerable from the file's own banner, exactly as it is for the triage.
 *
 * @return array<string, mixed>
 */
function derive_pipeline(): array
{
    $line = first_line_matching(read_tracked_file(PIPELINE), '/^\*\*Measured /');
    $sha = $line !== null && preg_match('/at `([0-9a-f]{7,40})`/', $line, $m) === 1 ? $m[1] : null;
    $rows = $line !== null && preg_match('/\*\*(\d+) row\(s\)\*\*/', $line, $r) === 1 ? (int) $r[1] : null;
    $held = $line !== null && preg_match('/(\d+) held/', $line, $h) === 1 ? (int) $h[1] : null;
    ...
    return [
        ...
        'commits_behind_main' => $counted['raw'],
        'commits_behind_effective' => $counted['effective'],
        'note' => 'generated by scripts/pipeline.php — regenerate it rather than editing it; a held row is '
            .'unscheduled, not invisible, and is counted here',
        'source' => PIPELINE,
    ];
}
...
function render(array $state): void
{
    ...
    if ($state['pipeline']['sha'] === null) {
        warn('pipeline', PIPELINE.' carries no provenance line. It is the single queue, so a missing '
            .'banner means the count below is not the tree.');
    } else {
        info('pipeline', sprintf(
            '%d row(s), %d held — %s, %s',
            (int) $state['pipeline']['rows'],
            (int) $state['pipeline']['held'],
            substr((string) $state['pipeline']['sha'], 0, 7),
            $state['pipeline']['commits_behind_effective'] === null
                ? $state['pipeline']['commits_behind_main'].' commits behind main (raw)'
                : $state['pipeline']['commits_behind_effective'].' commit(s) behind main that could produce '
                  .'a CI run, of '.$state['pipeline']['commits_behind_main'].' raw'
        ));
        note((string) $state['pipeline']['note']);
    }
    ...
    if ($state['triage']['commits_behind_main'] !== null) {
        ...
        //    few lines below warns when its sha will not resolve; this one printed NOTHING AT ALL,
        //    so a triage file naming a sha that is not on the trunk — a branch commit, a squashed
        //    one, a typo — was indistinguishable from a healthy file in the human report. Same
        //    measurement, and one of the two was silent about failing to make it.
        warn('triage staleness not measurable — '.TRIAGE.' names no sha, or names one that is not on '.
...
function render_d5(array $d5): void
{
    ...
    note('Presence of the marker is GATED (M65) by tests/Feature/Docs/BacklogProvenanceTest.php, so an');
