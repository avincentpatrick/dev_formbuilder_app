...
it('marks exactly the three provisioning metrics as gauges', function (): void {
    $gauges = array_values(array_filter(
        UsageMetric::cases(),
        static fn (UsageMetric $m): bool => $m->isGauge(),
    ));

    // In catalog order: storage_bytes, active_seats, forms_count.
    expect($gauges)->toBe([UsageMetric::StorageBytes, UsageMetric::ActiveSeats, UsageMetric::FormsCount]);
});
